...
d.tidy.resp.counts <- d.tidy.resps.classified.full %>%
  group_by(stim_id, superordinate, np_match, np_expectations, adj_polarity, specific, cat_cleaned) %>%
  count() %>%
...
d.tidy.resp.counts <- d.tidy.resp.counts %>%
  ...
  mutate(mode = cat_cleaned[which.max(n)])
...
distinctSuper %>%
  rowwise() %>%
  filter(stri_detect(produced_super, regex = ".s$")) %>%
...
d.tidy.phrase.w.original.mode.pl %>%
  filter((specific == 1) | (matchingSuper == 1)) %>%
  select(workerid, stim_id, degree, superordinate_pl = superordinate.freq, adj, adj_polarity, NP_sg = np, NP_pl = np_pl, np_expectations, response = mode_np, specific, subFreq = Frequencies.x, superFreq = Frequencies.y) %>%
