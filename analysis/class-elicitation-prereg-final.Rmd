...
df.trials.filtered <- df.trials %>%
  ...
  mutate(
   ...
   # np_match = gsub("podcast episode", "podcast", np_match),
    ...
    np_match = gsub("apartment building", "apartment", np_match),
    np_match = gsub("limousine", "limo", np_match),
    np_match = gsub("smartcar", "smart", np_match),
    np_match = gsub("conference center", "conference", np_match),
...
df.trials.filtered %>% 
  mutate(response = as.character(response),
         ...
         response = gsub("smart", "smartcar", response),
 ...
 #anti_join(stop_words)  %>% # remove stop words
  ...
  # check if response and seed NP match 
    specific = containsString_responses(as.character(cleanedPhrase), np_match, originalPhrase, np_match_us),
  # check if response and anticipated superordinate match
    matchingSuper = containsString_responses(as.character(cleanedPhrase), as.character(superordinate), originalPhrase, super_us)
...
d.tidy.resp.valid.resps <- d.tidy.phrase.w.original %>% rowwise() %>%
  subset(., !(originalPhrase %in% c("unsure", "short", "fast", "alexander", "alicia", "cheap", "cheep", "daniel", "dark", "darker", "david", "easy", "empire building",
                                    "eric", "gabriel", "anthony", "william", "john", "tom", "lucas", "michael", "cameron", "stephen", "lee", "greg", "robert", "chrales",
                                    "gary", "josh", "simon", "angela", "jennifer", "monica", "melinda", "veronica", "alicia", "maya", "sophie", "kim", "laura", "julia",
                                    "michelle", "stephanie", "claire", "kathleen", "jasmine", "tanya", "mika", "natalia", "barbell snatch", "bank of america",
                                    "exensive", "expensive", "finished", "flexibility", "hard", "health", "heavy", "hot cold", "hot", "cold", "lazy", "loud", "low",
                                    "narrow", "public", "quiet", "ripe" , "wide", "weak", "warm", "tall", "strong", "soft", "slow", "silent")))
```

``` {r}
# look at the raw responses
d.tidy.resp.analysis <- d.tidy.resp.valid.resps %>% filter((specific == 0) & (matchingSuper == 0)) %>% distinct(degree, superordinate, super_us, np, adj, np_match, np_match_us, cleanedPhrase, originalPhrase, specific, matchingSuper)
```
...
d.tidy.phrase.classified <- d.tidy.resp.valid.resps %>% 
  ...
  mutate(cat_cleanedPhrase = cleanedPhrase,
    ...
    #     cat_cleanedPhrase = gsub("bed", "bedboard", cat_cleanedPhrase),
         cat_cleanedPhrase = gsub("liquid", "drinks", cat_cleanedPhrase),
     ...
     #    cat_cleanedPhrase = gsub("oil smells", "oil paint", cat_cleanedPhrase),
         ...
         cat_cleanedPhrase = gsub("times", "times of day", cat_cleanedPhrase),
         ...
         cat_cleanedPhrase = gsub("rent", "rentals", cat_cleanedPhrase),
         ...
         cat_cleanedPhrase = gsub("thunderstoms", "thunderstorm", cat_cleanedPhrase),
        ...
        # cat_cleanedPhrase = gsub("blacks", "black paint", cat_cleanedPhrase),
         ...
         cat_cleanedPhrase = gsub("flower arrangements", "flowers", cat_cleanedPhrase),
   ...
   cat_cleanedPhrase = ifelse(superordinate == "times of day",  gsub("days", "times of day", cat_cleanedPhrase), cat_cleanedPhrase),
   ...
   cat_cleanedPhrase = gsub("episode", "podcast episode", cat_cleanedPhrase),
   ...
   cat_cleanedPhrase = gsub("baked", "dessert", cat_cleanedPhrase),
   ...
   cat_cleanedPhrase = gsub("bags trash", "trash can", cat_cleanedPhrase),
   ...
   cat_cleanedPhrase = gsub("cry frinch", "finch", cat_cleanedPhrase),
   cat_cleanedPhrase = gsub("driving lanes", "country lanes", cat_cleanedPhrase),
   ...
   cat_cleanedPhrase = ifelse(degree == "darkness",  gsub("time", np_match, cat_cleanedPhrase), cat_cleanedPhrase),
   ...
   cat_cleanedPhrase = gsub("spas", "sauna", cat_cleanedPhrase),
...
d.tidy.resp.analysis.update <- d.tidy.phrase.classified %>% filter((specific == 0) & (matchingSuper == 0))  %>% distinct(degree, superordinate, np, adj, np_match, cleanedPhrase, cat_cleanedPhrase, specific, matchingSuper)
```


```{r}
d.item.count <- d.tidy.phrase.classified %>%
...
d.sub.counts <- d.tidy.phrase.classified %>%
  ...
  count(specific)
d.sub.counts
d.consistent.items <- d.sub.counts %>% group_by(stim_id, np_expectations, adj_polarity) %>% mutate(nr_dp = sum(n)) %>% filter(n >= nr_dp-2)#filter( ((specific == 1) & (n >= nr_dp-2)) | ((specific == 0 ) & (n >= nr_dp-2))) # extract the consistent items 
d.consistent.items
...
d.tidy.phase.classifiedSuper <- d.tidy.phrase.classified %>%
  mutate(
    ...
    cleanedPhrase = gsub("animal homes", "dens", cleanedPhrase),
    cleanedPhrase = gsub("animal noises", "sounds", cleanedPhrase),
...
d.tidy.phase.classifiedSuper %>% filter((specific == 0) & (matchingSuper == 0)) %>% distinct(degree, superordinate, super_us, np, adj, np_match, np_match_us, cleanedPhrase, cat_cleaned, specific, matchingSuper) %>% View()
```


```{r}
d.tidy.phrase.specificMarking.bs <- d.tidy.phrase.classified %>% 
  mutate() %>%
