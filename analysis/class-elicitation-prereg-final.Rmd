...
containsString_responses <- function(strng, substrng, alt_substr){
  return(ifelse(length(strsplit(strng, substrng)[[1]]) > 1, 1, 
                ifelse( length(strsplit(alt_substr, strng)[[1]]) > 1, 1, 0)))
}
...
df.trials.filtered <- df.trials %>%
  ...
  mutate(
   ...
   # np_match = gsub("podcast episode", "podcast", np_match),
    ...
    np_match = gsub("patio", "patio", np_match)
    )
...
df.trials.filtered %>% 
  ...
  # check if response and seed NP match 
    specific = containsString_responses(as.character(cleanedPhrase), np_match, np_match_us),
  # check if response and anticipated superordinate match
    matchingSuper = containsString_responses(as.character(cleanedPhrase), as.character(superordinate), super_us)
...
d.tidy.resp.valid.resps <- d.tidy.phrase.w.original %>% rowwise() %>%
  mutate(originalPhrase = gsub("_", "", cleanedPhrase)) %>%
  subset(., !(originalPhrase %in% c("unsure", "short", "fast", "alexander", "alicia", "cheap", "cheep", "daniel", "dark", "darker", "david", "easy", "empire building",
                                    ...
                                    "michelle", "stephanie", "claire", "kathleen", "jasmine", "tanya", "mika", "natalia")))
```

``` {r}
# look at the raw responses
d.tidy.resp.analysis <- d.tidy.resp.valid.resps %>% filter((specific == 0) & (matchingSuper == 0)) %>% distinct(degree, superordinate, super_us, np, adj, np_match, np_match_us, cleanedPhrase, specific, matchingSuper)
```
...
d.tidy.phrase.unexpected.resps1 <- d.tidy.resp.analysis %>% 
  ...
  mutate(cat_cleanedPhrase = cleanedPhrase,
   ...
   #      cat_cleanedPhrase = gsub("mouse holes", "mouse den", cat_cleanedPhrase),
        ...
        # cat_cleanedPhrase = gsub("bears den", "bear den", cat_cleanedPhrase),
         ...
         cat_cleanedPhrase = ifelse(degree == "darkness",  gsub("color", "paint", cat_cleanedPhrase), cat_cleanedPhrase),
  ...
  mutate(specific = containsString_responses(as.character(cat_cleanedPhrase), as.character(np_match), np_match_us), 
         matchingSuper = containsString_responses(as.character(cat_cleanedPhrase), as.character(superordinate), super_us)
         ) %>%
