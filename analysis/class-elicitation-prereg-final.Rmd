...
d.tidy.resp.valid.resps %>% 
  ...
  mutate(
    ...
    response_lemma = str_replace_all(response_synonym, lemmatization_set)
    ) %>%
...
d.matchedResponses %>% 
  ...
  distinct(stim_id, context_sentence, np_expectations, np_match, adj_polarity, originalResponse, meanResp) %>% View()
...
d.tidy.resp.analysis %>% 
  distinct(degree, superordinate, super_us, np, adj, np_match, originalResponse, originalPhrase, specific, matchingSuper) %>% 
  View()
```
