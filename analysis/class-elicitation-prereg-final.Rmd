...
df.trials.filtered.spellsyn %>% 
  ...
  # check if response and seed NP match 
    specific = ifelse(originalPhrase == "times of day", # catch these responses because otherwise classified as subordinate for "day" / "strawberries" etc
...
d.tidy.phrase.w.original %>% filter(grepl("similar", originalResponse, ignore.case=T)) -> d.uncertain.resps # 15 responses
...
d.tidy.phrase.handClassified <- d.tidy.resp.analysis %>% 
  ...
  mutate(cat_cleanedPhrase = cleanedPhrase,
         ...
         cat_cleanedPhrase = ifelse( superordinate == "cookware", gsub("pans", "frying pan", cat_cleanedPhrase),
                                     cat_cleanedPhrase),
         ...
         cat_cleanedPhrase = ifelse(superordinate == "times of day",  ifelse(cat_cleanedPhrase == "days", np_match,
                                                                             cat_cleanedPhrase), cat_cleanedPhrase),
...
``` {r}
df.frequencies <- read_csv('../data/class-elicitation-prereg-final/class-elicitation-final-full_analysis_w_pl_np.csv')
d.tidy.phrase.w.original.mode.pl <- d.tidy.resps.classified.full.adjust.Super %>%
