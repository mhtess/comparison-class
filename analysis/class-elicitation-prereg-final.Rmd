...
To further improve the response classification, we also replace some responses which are obviously sinonymous to the provided subordinate or superoridnate NPs given the context.
...
hand_classification <- c("'" = "",
                         ...
                         "animals aquiariums" = "animals",
                         "animals forest" = "animals",
                         "animals forrest" = "animals",
                         "animals forest" = "animals",
                         "pieces of"= "",
   "alcohol drinks" = "alcoholic drinks",
   "animal houses" = "dens",
   ...
   "machinery"= "machines",
   "meals resataurants"= "restaurants",
   "market foods"= "food",
   "previous cars"= "cars",
   ...
   "move furniture"= "furniture",
   "movies"= "movie",
   "music instrument"= "muical instruments",
   "music sounds" = "music",
   "noises" = "noise",
   "noise area" ="noise",
   "noise forest night" = "noise",
   "noise hears" = "noise",
   "noise lake" = "noise",
   "noise park" = "noise",
   "noise  nearby" = "noise",
   "object kitchen" = "objects",
   "objects house" = "objects",
   "objects kitchen" = "objects",
   "objects lift" = "objects",
   "objects lifted" = "objects",
   "objects sitting" = "objects",
   "objects store" = "objects",
   "objects terrarium" = "objects",
   "openings earth" = "openings",
   "objects house" = "objects",
   "papers writing scripts" = "paper", 
   "parks" = "park",
   "pathways" = "path",
   ...
   "selling price"= "price",
   "shades stronger characteristic"= "paints",
   "snack foods"= "snacks",
   "sofas"= "couch",
   "satues"= "statues",
   "songs played" = "songs", 
   ...
   "alochols" =  "alcoholic drinks",
   "random wood" = "wood",
   "areas furniture" = "areas", 
   "areas kitchen" = "areas",
   "areas nearby" = "areas",
   "areas of house" = "areas",
   "banks" = "bank",
"areas of population" = "areas",
  ...
  "pick up" = "pick-up", 
   "places htey" = "venues",
   "places forest" = "forest", 
   "places darker" = "places",
   "places quiet"= "places",
   "places met" = "venue",
   "places live" = "rentals",
   "places sit" = "furniture",
  ...
  "temperatures room" = "temperature",
   ...
   "text" = "script", 
   "things hardware store" = "things",
   "things craft shop" = "things", 
   "things lake" = "things",
   "things eaten" = "food",
   "things lifted" = "things",
   "things air" = "things",
   "things airport" = "things", 
   "things forest" = "things",
   "things kitchen" = "things",
   "things ocean" = "things",
   "things room" = "things",
   "things store" = "things",
   "things sky" = "things",
   "things terrarium" =  "things",
  "things table" = "things",
   "things lake" = "things",
   "things fast"= "things",
   "things short" = "things",
   "things move" = "things",
    "things pick up" = "things",
   "tings weight"= "things",
   "trucks" = "truck",
...
d.tidy.phrase.handClassified <- d.tidy.resp.analysis %>%
                      mutate( response = as.character(response),
                        response_lemma = str_replace_all(response, hand_classification),
        
         response_lemma = ifelse( superordinate == "cookware", 
                                     gsub("pans", "frying pan", response),
                                     response_lemma),
         response_lemma = gsub("illunation", np_match, response),
         response_lemma = gsub("illumination", np_match, response),
         response_lemma = ifelse(superordinate == "desserts",  gsub("chocolate", "chocolate fondue",
                                                                       response), response_lemma),
         response_lemma = ifelse(superordinate == "times of day",  
                                    ifelse(response_lemma == "days", np_match,
                                                                             response), response_lemma),
         response_lemma = ifelse(superordinate == "writing utensils",  gsub("ink", "pen", response),
                                    response_lemma),
         response_lemma = ifelse(superordinate == "buildings",  gsub("apartments", "apartment building",
                                                                        response), response_lemma),
        
         response_lemma = gsub("brands", np_match, response),
         response_lemma = gsub("butchershops", np_match, response),
         response_lemma = ifelse(superordinate == "places",  gsub("baths", "sauna", response),
                                    response_lemma),
         response_lemma = ifelse(superordinate == "alcoholic drinks",  gsub("bottles", "wine", response),
                              response_lemma),
         response_lemma = ifelse(superordinate == "furniture",  gsub("cans", "trash cans", response),
                                    response_lemma),
         response_lemma = ifelse(degree == "loudness",  gsub("child", "baby", response),
                                    response_lemma),
         response_lemma = ifelse(degree == "weight",  gsub("children", "kid", response),
                                    response_lemma),
         response_lemma = ifelse(degree == "weight",  gsub("childern", "kid", response),
                                    response_lemma),
         response_lemma = ifelse(degree == "temperature_h",  gsub("chocolate", "chocolate fondue",
                                                                     response), response_lemma),
         response_lemma = ifelse(degree == "darkness",  gsub("duck", "dusk", response),
                                    response_lemma),
         
         response_lemma = gsub("compared store", np_match, response),
        
         response_lemma = ifelse(degree == "darkness",  gsub("color", np_match, response),
                                    response_lemma),
         response_lemma = ifelse(degree == "darkness",  gsub("fur color", np_match, response),
                                    response_lemma),
         response_lemma = ifelse(degree == "darkness",  gsub("fur", np_match, response),
                                    response_lemma),
         response_lemma = ifelse(degree == "temperature_h",  gsub("day", np_match, response),
                                    response_lemma),
         response_lemma = ifelse(degree == "loudness_n",  gsub("day", np_match, response),
                                    response_lemma),
         response_lemma = ifelse(degree == "price",  gsub("day", np_match, response), response_lemma),
         response_lemma = ifelse(degree == "price",  gsub("flats", "apartments", response),
                                    response_lemma),
         response_lemma = ifelse(degree == "speed",  gsub("kid", np_match, response), response_lemma),
         response_lemma = ifelse(degree == "height",  gsub("kid", np_match, response), response_lemma),
         response_lemma = ifelse(degree == "loudness",  gsub("kid", np_match, response),
                                    response_lemma),
         response_lemma = ifelse(degree == "strength",  gsub("kid", np_match, response),
                                    response_lemma),
         response_lemma = ifelse(degree == "weight",  gsub("kid", np_match, response), response_lemma),
         response_lemma = ifelse(degree == "width",  gsub("lane", np_match, response), response_lemma),
         response_lemma = ifelse(degree == "darkness",  gsub("light", np_match, response),
                                    response_lemma),
         response_lemma = ifelse(degree == "height",  gsub("mall", np_match, response),
                                    response_lemma),
        response_lemma = ifelse(degree == "speed_f",  gsub("planes", np_match, response),
                                   response_lemma),
        response_lemma = ifelse(degree == "height",  gsub("player", np_match, response),
                                   response_lemma),
        response_lemma = ifelse(degree == "temperature_h",  gsub("rinks", np_match, response),
                                   response_lemma),
       response_lemma = ifelse(degree == "temperature_h",  gsub("saturday", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(superordinate == "animals",  gsub("shade", np_match, response),
                                 response_lemma),
       response_lemma = ifelse(degree == "loudness_n",  gsub("time", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(degree == "temperature_h",  gsub("time", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(degree == "speed",  gsub("time", np_match, response), response_lemma),
       response_lemma = ifelse(degree == "price",  gsub("time", np_match, response), response_lemma),
       response_lemma = ifelse(degree == "darkness",  gsub("time", "times of day", response),
                                  response_lemma),
       response_lemma = ifelse(degree == "price",  gsub("store", np_match, response), response_lemma),
       response_lemma = ifelse(degree == "price",  gsub("shop", np_match, response), response_lemma),
       response_lemma = ifelse(degree == "size",  gsub("store", np_match, response), response_lemma),
       response_lemma = ifelse(degree == "loudness_n",  gsub("store", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(degree == "length_duration",  gsub("store", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(degree == "temperature_h",  gsub("sip", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(superordinate == "animals",  gsub("color", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(superordinate == "dogs",  gsub("color", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(superordinate == "cakes",  gsub("color", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(superordinate == "flowers",  gsub("color", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(superordinate == "materials",  gsub("color", np_match, response),
                                  response_lemma),
       response_lemma = ifelse(superordinate == "birds",  gsub("color", np_match, response),
                                 response_lemma),
          
         response_lemma =ifelse(degree == "darkness",  gsub("times", "times of day", response),
                                   response_lemma),
         response_lemma = gsub("top shelf options", "liquor", response),
         response_lemma = ifelse(degree == "price", gsub("rent", "rentals", response), response_lemma),
        
         response_lemma = ifelse(superordinate == "paints",  gsub("color", "paint", response),
                                    response_lemma),
         response_lemma = ifelse(superordinate == "seasons",  gsub("day", "seasons", response),
                                    response_lemma),
         response_lemma = ifelse(superordinate == "statues",  gsub("material", "statues", response),
                              response_lemma),
         response_lemma = ifelse(superordinate == "insects",  gsub("bug", "insect", response),
                                    response_lemma),
         response_lemma = ifelse(superordinate == "cookware",  gsub("cook", "cookware", response),
                                    response_lemma),
         response_lemma = ifelse(superordinate == "venues",  gsub("auditoriums", "concert", response),
                                    response_lemma ),
        
   response_lemma = ifelse(degree == "speed", gsub("appliances", "cookware", response), response_lemma),
    response_lemma = gsub("market", np_match, response),
    response_lemma = gsub("playing", np_match, response),
    response_lemma = gsub("saturday nights", np_match, response),
    response_lemma = ifelse(superordinate == "places", gsub("venues", "places", response), response_lemma),
    response_lemma = ifelse(superordinate == "concerts", gsub("singing", "concerts", response), response_lemma),
    response_lemma = ifelse( degree == "strength", gsub("body", "athlete", response), response_lemma),
    response_lemma = ifelse( superordinate == "materials", gsub("cloth", "fabric", response), response_lemma),
    response_lemma = ifelse( superordinate == "shoes", gsub("shes", "shoes", response), response_lemma),
    response_lemma = ifelse( superordinate == "concerts", gsub("show", "concert", response), response_lemma),
    response_lemma = ifelse( superordinate == "audios", gsub("show", "audio", response), response_lemma),
    response_lemma = ifelse( superordinate == "statues", gsub("metal", "statue", response), response_lemma),
    response_lemma = ifelse( superordinate == "cookware", gsub("method", "cookware", response), response_lemma),
    response_lemma = ifelse( superordinate == "venues", gsub("night", np_match, response), response_lemma),
    response_lemma = ifelse( superordinate == "doorways", gsub("opening", "doorway", response), response_lemma),
    response_lemma = ifelse( superordinate == "dens", gsub("opening", "den", response), response_lemma),
    response_lemma = ifelse( superordinate == "times of day", gsub("area", np_match, response), response_lemma),
    response_lemma = ifelse( superordinate == "times of day", gsub("places", np_match, response), response_lemma),
    response_lemma = ifelse( superordinate == "times of day", gsub("area", np_match, response), response_lemma),
    response_lemma = ifelse( superordinate == "times of day", gsub("street", np_match, response), response_lemma),
    response_lemma = ifelse( superordinate == "rentals", gsub("places", "rentals", response), response_lemma),
    response_lemma = ifelse( superordinate == "food", gsub("dishes", "food", response), response_lemma),
    response_lemma = ifelse( superordinate == "restaurants", gsub("places", "restaurants", response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "restaurants", gsub("places dinner", "restaurants", response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "alcoholic drinks", gsub("places", np_match, response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "doorways", gsub("path", "doorway", response), response_lemma),
    response_lemma = ifelse( superordinate == "dens", gsub("passage", "den", response), response_lemma),
    response_lemma = ifelse( superordinate == "roads", gsub("path", "road", response), response_lemma),
    response_lemma = ifelse( superordinate == "waterways", gsub("path", "waterways", response), response_lemma),
    response_lemma = ifelse( superordinate == "audios", gsub("performance", "audio", response), response_lemma),
    response_lemma = ifelse( superordinate == "audios", gsub("music", "audio", response), response_lemma),
    response_lemma = ifelse( superordinate == "materials", gsub("hardware", "materials", response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "audios", gsub("performance", "venues", response), response_lemma),
    response_lemma = ifelse( superordinate == "locations", gsub("places house", "locations", response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "locations", gsub("places kitchen", "locations", response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "locations", gsub("refrigerator", "freezer", response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "doorways", gsub("places house", "doorways", response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "floors", gsub("surfaces", "floors", response), response_lemma),
    response_lemma = ifelse( superordinate == "races", gsub("tracks", "races", response), response_lemma),
    response_lemma = ifelse( superordinate == "restaurants", gsub("menu", "restaurants", response),
                            response_lemma),
    response_lemma = ifelse( superordinate == "walls", gsub("building materials", "walls", response),
                            response_lemma),
   
    response_lemma = ifelse( cleanedPhrase == "_cries_", np_match_us, cleanedPhrase),
    response_lemma = ifelse( cleanedPhrase == "_crying_", np_match_us, cleanedPhrase),
    response_lemma = ifelse( cleanedPhrase == "_episodes_", np_match_us, cleanedPhrase),
    response_lemma = ifelse( cleanedPhrase == "_car_", "_cars_", cleanedPhrase),
    response_lemma = ifelse( cleanedPhrase == "_map_", "_maps_", cleanedPhrase),
    response_lemma = ifelse( cleanedPhrase == "_pots_", "_cookware_", cleanedPhrase),
   response_lemma = ifelse( cleanedPhrase == "_settings_", super_us, cleanedPhrase),
    response_lemma = ifelse( cleanedPhrase == "_price_", np_match_us, cleanedPhrase),
    response_lemma = ifelse( cleanedPhrase == "_pieces_", np_match_us, response),
    response_lemma = ifelse( np == "sauna", gsub("spa", "sauna", response), response_lemma),
    response_lemma = ifelse( np == "bedboard", gsub("board", "bedboard", response), response_lemma),
    response_lemma = gsub("shopping", np_match, response),
    response_lemma = gsub("works of art", superordinate, response)
  ...
  mutate( cleanedPhrase = paste("_", as.character(response_lemma), "_", sep=""),
    #response_lemma_cleaned = gsub("_", "", response_lemma),
    specific = containsString_responses(as.character(cleanedPhrase), as.character(np_match), response_lemma,  np_match_us), 
         matchingSuper = containsString_responses(as.character(cleanedPhrase), as.character(superordinate), response_lemma, super_us)
         ) 
```
...
d.tidy.phrase.handClassified %>% filter((specific == 0) & (matchingSuper == 0)) -> d.nonMatching.resps # %>% distinct(degree, superordinate, super_us, np, adj, np_match, np_match_us, originalResponse, cleanedPhrase, cat_cleaned, specific, matchingSuper) 
...
d.scaleResps <- d.nonMatching.resps %>% rowwise() %>% mutate(degree_match = degree,
...
d.HandClassified.subs.wCC <- anti_join(d.tidy.phrase.handClassified, d.nonMatching.resps) %>% select(workerid, stim_id, degree, context_sentence, superordinate, np, np_expectations, adj, adj_polarity, np_match, response, response_lemma, specific, matchingSuper)
