...
my_stop_words <- stop_words %>% rowwise() %>%
    subset(!(stop_words$word %in% c("rooms", "year", "years", "side", "sides", "area", "of", "up", "great", "things", "places", "thing", "room", "areas",
                                    "part", "parts", "works", "way", "ways", "-", "show", "shows")))
...
d.tidy.resp.valid.resps <- anti_join(df.trials.filtered, d.tidy.resps.catch) %>% rowwise() %>%
  subset(., !(response %in% c("unsure", "long", "short", "tall", "dark", "light", "fast", "quick", "slow", "hard", "soft", "loud", "noisy", "quiet", 
                                    ...
                                    "public", "ripe" , "silent", "baked", "A bottle of wine at a liquor store was expensive", "A piece of cotton in craft shop was light", "A piece of plastic in hardware store was soft", 
...
d.tidy.phrase.handClassified <- d.tidy.resp.analysis %>% 
  ...
  mutate(cat_cleanedPhrase = cleanedPhrase,
         ...
         cat_cleanedPhrase = gsub("road widths", "width", cat_cleanedPhrase),
