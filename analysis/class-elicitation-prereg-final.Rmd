...
The analysis proceedes from simplest processing steps to more sophisticated processing steps, leading to the output of dataframes containing data passing these different processing steps. At the end, data visualisations and regression models can be found.
...
d.tidy.auto.no_similar <- anti_join(
  d.tidy.auto,
  d.uncertain.resps
)

# extract the automatically matched data (out of 26.552 responses (post exclusion) 22.117 are matched )
d.tidy.auto.no_similar.match <- d.tidy.auto.no_similar %>%
  filter((specific == 1) | (matchingSuper == 1))


n.automatic.responses <- d.tidy.auto.no_similar.match %>%
  ungroup() %>%
  count() %>%
  pull(n)
...
If the response was classified as subordinate (i.e. _specific_), it is turned into be the corresponding NP, otherwise it is turned into the corresponding superordinate.

``` {r}
d.tidy.auto.no_similar.match.adjust_super <- d.tidy.auto.no_similar.match %>%
  mutate(
...
d.tidy.auto.non_match %>%
  ...
  View()
```
First, we further lemmatize the non-matching responses and correct for misspellings to extract the responses which were produced by at least 3 participants for the respective stimulus. 
The responses are lower-cased. The lemmatization and misspeling correction is read into a dataframe and converted into a named list to use with `str_replace_all`. 
...
# apply lematizations
hand_class_lemmatization_df <- read_csv('../data/class-elicitation-prereg-final/hand_classification_lemmatization_dict.csv') %>% 
  rowwise() %>%
  mutate(
    hand_class_replacements = ifelse(is.na(hand_class_replacements), "",
                                     hand_class_replacements))

hand_lemmatization <- pull(hand_class_lemmatization_df,
                           hand_class_replacements) %>%
...
The lemmatized and corrected reponses are counted by-stimulus; we keep those which were produced by at least 3 participants. 890 non-matched responses (20.7%) were produced less thna 3 times and are dropped. 
...
d.tidy.auto.non_match.grab3 <- d.tidy.auto.non_match.lemma.counts %>%
  right_join(., d.tidy.auto.non_match, by=c("stim_id", "response_hand_lemma")) %>% filter(n >= 3)
...
Some responses fail to establish correct reference, e.g. participants referred to the wrong entity mentioned in the context sentence: given the sentence "You see an adult lift a box" and the comment "They're strong", when asked who the adult is strong relative to subjects responded "relative to other boxes". Since these 'failed-reference' responses often mention nouns in the context sentence, we extract these responses by looking for content words from the contexts. This approach is rather conservative also grabs responses describing e.g. the place which could be potentially interpreted as referencing other subordinate instances is different settings. 
``` {r}
...
d.tidy.auto.non_match.grab3.failedRef <- d.tidy.auto.non_match.grab3.removeStopWords %>%
  unnest_tokens(word, response_hand_lemma) %>%
  filter(grepl(word, clean_context_sentence, ignore.case = T )) %>%
  group_by(workerid, trial_num) %>%
  mutate(wordNumber = paste("word_", row_number(), sep = "")) %>%
  spread(wordNumber, word) %>%
  unite(response_hand_lemma, starts_with("word_"), sep = " ") %>%
  mutate(response_hand_lemma = gsub(" NA", "", response_hand_lemma))
```

The filing-reference responses are excluded (587 responses, ). Other responses are classified using a dictionary of manual classification read from a dataframe:
...
d.tidy.auto.non_match.hand_classified <- d.tidy.auto.non_match.grab3.noFailedRef %>%
  mutate(
    ...
    response = as.character(response),
    response_lemma_hand = str_replace_all(response_hand_lemma,
                                          hand_classification)
  ) %>%
  mutate(
...
d.tidy.auto.non_match.hand_classified %>%
  ...
  distinct(stim_id, superordinate, np, adj, response_hand_lemma, response_lemma_hand) %>% View()
```
