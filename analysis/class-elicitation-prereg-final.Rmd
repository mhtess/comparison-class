...
d.tidy.phrase.handClassified <- d.tidy.resp.analysis %>%
                      mutate( response_lemma = str_replace_all(response, hand_classification), 
...
#    response_lemma = ifelse( cleanedPhrase == "_pieces_", np_match_us, response),
  ...
  mutate( response_lemma_cleaned = gsub("_", "", response_lemma),
    specific = containsString_responses(as.character(response_lemma), as.character(np_match), response_lemma_cleaned,  np_match_us), 
         matchingSuper = containsString_responses(as.character(response_lemma), as.character(superordinate), response_lemma_cleaned, super_us)
         ) 

# check unclassified responses left
d.tidy.resp.analysis.update <- d.tidy.phrase.handClassified %>% filter((specific == 0) & (matchingSuper == 0))  %>%  distinct(np, adj, np_match, response_lemma, cleanedPhrase, specific, matchingSuper)
```
...
d.tidy.phase.classifiedSuper <- d.tidy.phrase.w.original2 %>% #d.tidy.resp.analysis.update %>%
  mutate() %>%
  ...
  rename(originalResponse = response)
...
d.AutoClassified <- d.matchedResponses  %>% rename(originalResponse = response) %>% select(stim_id, degree, context_sentence, superordinate, np, np_expectations, adj, adj_polarity, np_match, originalResponse, cat_cleaned = originalPhrase, specific, matchingSuper)
