...
Suppose you walk into a coffee shop and overhear someone say “the coffee is warm.” What do you think they meant by that? If the person is drinking a [cappuccino](https://en.wikipedia.org/wiki/Cappuccino), warm might mean around 60&deg;C or 160&deg;F. In contrast, if the person is drinking an [iced coffee](https://en.wikipedia.org/wiki/Iced_coffee), warm might mean around 15&deg;C or 59&deg;F. It is apparent that in order for us to ground a quantitative interpretation of the gradable adjective “warm,” we need to know the set of entities that coffee belongs to from which we can derive a standard for "warm." In other words, we are interested in finding the set that contains other entities that make the coffee in this particular utterance "warm": we are interested in finding the comparison class.

## What are comparison classes?

**Comparison classes** are the implicitly or textually determined sets that relativize the standard for some gradable adjective. In our previous example, the comparison class would have been either the $C$, set of all cappuccinos, or $I$, the set of all iced coffees. A standard for "warm" could then be obtained by performing some operation on the scale associated with either of these sets, such as calculating the mean temperature over the set of cappuccinos.

Before formalizing this any further, I will introduce some notations and definitions that underlie the theory of comparison classes. When considering the utterance "the coffee is warm," the subject of the sentence is clearly "coffee."

A **quasi-order** is a binary relation on a particular set and has the properties of reflexivity and transitivity (see [here](http://www.inf.ed.ac.uk/teaching/courses/dmmr/slides/13-14/Ch2.pdf) for a more detailed discussion on relations in set theory). Without any context or restriction, this is a relation on the set $\mathbb{U}$, known in discrete mathematics to be the _universe of discourse_ (i.e. the set of all things). Below to the left is a graphical interpretation of a quasi-order: 

<center>![](../img/quasi-and-linear-order.png)</center>
<center><font size="2">Figure 1: The graph on the left represents a quasi-order and the graph on the right represents a quasi-order that was converted to a linear order by means of collapsing the nodes into their equivalence classes.</font></center><br>

Though these quasi-orders are instantiated with gradable adjectives, such as "warm," and can describe the relationship between various entities in a set, it is not enough to call them scales since 

From the previous vignette and using the approach from @bale2011scales, we can set up a _quasi-order_ of "warm," a binary relation on the comparison class, as follows:
