...
df.cc.infer <- df.cc.infer %>% rowwise() %>% mutate(test = paste(degree, superordinate_pl, NP_sg, NP_pl, superordinate_sg, sep=","))
left_join(df.adj.w.mode, df.cc.infer, by=c("test"="test")) %>% View()
