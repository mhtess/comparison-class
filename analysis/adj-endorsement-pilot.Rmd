...
df.adj.w.mode <- df.adj.w.mode %>% rowwise() %>% mutate(np_pl = insert_plural_np(item))
```

``` {r}
df.adj.w.mode <- df.adj.w.mode %>% mutate(superordinate = ifelse(item == "jockey", "people", superordinate ),
        ...
        np_pl = gsub("highway", "highways", np_pl),  
        np_pl = gsub("side road", "side roads", np_pl), 
        np_pl = gsub("downtown street", "downtown streets", np_pl), 
...
df.adj.w.mode %>% select(workerid, degree, superordinate, np_pl, item_cond, key, val) %>% 
  write_csv( '../data/pilot-adj-endorsement-1/pilot-adj-endorsement-1-super-PL.csv')
```