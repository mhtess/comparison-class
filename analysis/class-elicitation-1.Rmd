...
```{r}
d.subject <- read.csv(paste(data.path, "class-elicitation-full-subject_information.csv", sep = ""))
...
#df.summary %>% #filter(condition == "contextWithSuper") %>%
  ggplot(., 
...
ggsave(file = paste(project.path, "analysis/fig/classElicitation-1-n264.pdf", sep = ""), 
       width = 12, height = 6)
options(warn=0)
```
