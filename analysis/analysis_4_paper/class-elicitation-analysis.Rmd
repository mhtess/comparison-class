...
auto_data <- read_csv("../../data/class-elicitation-prereg-final/auto-classified-data-w-freqs-final.csv")
```
...
d.tidy.phrase.specificMarking.bs <- data %>%
  group_by(np_expectations, adj_polarity) %>%
  tidyboot_mean(column = specific)

d.tidy.phrase.specificMarking.bs.item <- data %>%
...
```{r}
d.tidy.clean.responses.fac <- data %>%
  mutate(
