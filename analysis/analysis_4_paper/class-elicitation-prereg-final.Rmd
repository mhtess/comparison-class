...
# extract the automatically matched data (out of 26.552 responses (post exclusion) 22.339 are matched )
...
d.tidy.auto.non_match.grab3.failedRef <- d.tidy.auto.non_match.grab3.removeStopWords %>%
  ...
  filter(grepl(word, clean_context_sentence)) %>% # MH: I THINK THIS IS MISSING PLURALS (e.g., the first row of this df is about "scripts", but the context sentence only has the word "script" in it)
  group_by(workerid, trial_num) %>%
  ...
  filter(!(
      ...
      (resps_matching_context == "podcast") |
      (resps_matching_context == "coat") |
      (response_hand_lemma == "bouquet") |
      (response_hand_lemma == "flower bouquet") |
      (response_hand_lemma == "mall") |
      (response_hand_lemma == "house") |  
      (response_hand_lemma == "places to sit") |
      (response_hand_lemma == "parts of the kitchen") |  
      (response_hand_lemma == "items in the shop") |
      (response_hand_lemma == "dinner") |
      (response_hand_lemma == "restaurant price") |  
      ...
      (response_hand_lemma == "gate") |
      (response_hand_lemma == "menu") |
      (response_hand_lemma == "weight lifters") |  
      (response_hand_lemma == "music") 
  ))
```


The reference-failing responses are excluded (288 responses). Remaining responses are classified using a dictionary of manual classification read from a dataframe:
...
d.tidy.phrase.w.original.mode.pl %>% # this is called differently now 
  filter((specific == 1) | (matchingSuper == 1)) %>%
