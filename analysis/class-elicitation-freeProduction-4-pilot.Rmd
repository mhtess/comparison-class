...
d.tidy.resp.counts <- d.tidy.phrase.w.original %>% 
  group_by(degree, superordinate, np, specific, adj, cleanedPhrase) %>% count() %>% 
...
# in column mode
d.tidy.resp.counts <- d.tidy.resp.counts %>% group_by(np, specific) %>% mutate(mode = cleanedPhrase[which.max(n)])
...
d.tidy.phrase.w.original.mode.pl <- d.tidy.phrase.w.original.mode.pl %>%
  left_join(., df.frequencies, by = c("mode_np" ="Nps")) %>% # add subordinate frequencies
...
write_csv(df.mode.final, '../data/pilot-classElicitation-free-4/class-elicitation-free-4-cleaned-data-w-sub-pl-update.csv')
```
