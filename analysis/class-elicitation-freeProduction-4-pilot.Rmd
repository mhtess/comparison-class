...
# paste singular superordinates to match it to the frequency data
...
# df.trials.w.super <- df.trials %>% 
#   rowwise() %>% 
#   mutate(superordinate = insert_super(np))
...
insert_plural_np = function(np) {
  ...
  } else if (np %in% c("six-pack of beer", "bottle of wine")) {
    np_pl = "beers"
  } else if (np %in% c("bottle of wine")) {
    np_pl = "bottles of wine"
...
df.trials.filtered %>% 
  ...
  select(workerid, trial_num, stim_id, context, np_positiveness, adj, degree, superordinate, adj_positiveness, np, np_pl, np_match, cleanedPhrase, originalResponse, specific, matchingSuper) -> d.tidy.phrase.w.original
...
``` {r}
d.tidy.phrase.w.original.mode <- d.tidy.phrase.w.original %>% subset(., !(np_pl %in% c("days", "dusks", "nights"))) %>%
  mutate(np_for_freq = np_pl,
        ...
        np_for_freq = gsub("highway", "highways", np_for_freq),  
        np_for_freq = gsub("side road", "side roads", np_for_freq), 
        np_for_freq = gsub("downtown street", "downtown streets", np_for_freq), 
       ...
       # superordinate = gsub("readings", "books",  superordinate),
       # superordinate = gsub("roads", "ways", superordinate),
    #    superordinate = gsub("rooms", "areas", superordinate),
      #   superordinate = gsub("floors", "surfaces", superordinate)
         )



d.tidy.phrase.w.original.mode <- d.tidy.phrase.w.original.mode %>% 
  #left_join(., df.frequencies, by = c("np_for_freq" ="NPs")) %>% left_join(., df.frequencies, by=c("superordinate"="NPs")) #%>%View()
...
``` {r}
# insert plural nps
d.tidy.phrase.w.original.mode.pl <- d.tidy.phrase.w.original.mode %>% 
  rowwise() %>% 
  mutate(np_pl = insert_plural_np(np))

d.tidy.resp.counts.pl <- d.tidy.phrase.w.original.mode.pl %>% subset(., !(np_pl %in% c("days", "dusks", "nights"))) %>%
  mutate(np_for_freq = np_pl,
        np_for_freq = gsub("carpets", "carpet floors", np_for_freq),
        np_for_freq = gsub("tiles", "tile floors", np_for_freq),
        np_for_freq = gsub("wood", "wood floors", np_for_freq),
  ...
  left_join(., df.frequencies, by = c("mode_np" ="NPs")) %>% # add sub freqs
  left_join(., df.frequencies, by=c("superordinate"="NPs"))   # add super freqs
...
#df.plural.std <- cbind(df.plural.02, df.plural.01, df.plural.04, df.plural.01, df.plural.07)
#df.plural.std <- cbind(df.plural.01, df.plural.mh, df.plural.ratio)
...
```{r}
#write_csv(d.tidy.phrase.w.original, "../data/pilot-classElicitation-free-4/class-elicitation-free-4-pilot-trials-clean_filtered.csv")
```
...
d.tidy.resp.counts <- d.tidy.phrase.w.original.mode %>% 
  group_by(degree, superordinate, np_for_freq, specific, adj, cleanedPhrase) %>% count() %>% 
  mutate(n_unique_resps = n)

#new.modes <- d.tidy.phrase.w.original %>% 
 # group_by(degree, superordinate, np_match, specific, adj, cleanedPhrase) %>% count() %>%
  #mutate(n_unique_resps = n) %>% group_by(np_match, specific) %>% mutate(mode = cleanedPhrase[which.max(n)])


d.tidy.resp.counts.web <- d.tidy.resp.counts %>% group_by(np_for_freq, specific) %>% mutate(mode = cleanedPhrase[which.max(n)])
```

``` {r}
# analysis of modal responses
d.tidy.resp.counts.web <- d.tidy.resp.counts.web %>% rowwise() %>%  
...
df.mode <- left_join(d.tidy.phrase.w.original.mode, d.tidy.resp.counts.web) %>% 
 # subset(., !(np_m %in% c("day", "dusk", "night"))) %>%
    ...
    #     np_for_freq = gsub("fall", "fall day", np_for_freq),
     #    np_for_freq = gsub("summer", "summer day", np_for_freq),
      #   np_for_freq = gsub("basketball", "basketball player", np_for_freq),
       #  superordinate_sg = ifelse(np_for_freq == "golfer", "people", superordinate_sg),
        # superordinate_sg = ifelse(np_for_freq == "jockey", "people", superordinate_sg),
    #     superordinate_sg = ifelse(np_for_freq == "basketball player", "people", superordinate_sg),
     #    superordinate_sg = ifelse(np_for_freq == "walker", "people", superordinate_sg),
      #   superordinate_sg = ifelse(np_for_freq == "runner", "people", superordinate_sg),
       #  superordinate_sg = ifelse(np_for_freq == "jogger", "people", superordinate_sg)
        
        # )%>% 
  mutate(mode_np = ifelse(specific == 1, np_for_freq, superordinate))%>%  
  left_join(., df.frequencies, by = c("np_for_freq" ="NPs")) %>% # add sub freqs
 left_join(., df.frequencies, by=c("superordinate"="NPs")) # add super freqs

df.mode.final <- df.mode %>% select(workerid, degree, superordinate, adj, adj_positiveness, np_for_freq, mode_np, specific, Frequencies.x, Frequencies.y) %>% mutate(superordinate = superordinate, NP = np_for_freq,  response = mode_np, subFreq = Frequencies.x, superFreq = Frequencies.y) %>% select(workerid, degree, superordinate, adj, adj_positiveness, NP, response, specific, subFreq, superFreq)

write_csv(df.mode.final, '../data/pilot-classElicitation-free-4/class-elicitation-free-4-cleaned-data-w-sub-pl.csv')
...
rs.glm.brm <- brm(
  formula = specific ~ np_positiveness * adj_positiveness + 
    ...
    (1 +  np_positiveness*adj_positiveness | superordinate),
