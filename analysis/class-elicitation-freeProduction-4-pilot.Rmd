...
df.trials.filtered %>% 
  ...
  select(workerid, trial_num, stim_id, context, np_positiveness, adj, degree, superordinate, adj_positiveness, np, np_match, cleanedPhrase, originalResponse, specific, matchingSuper, superordinate_sg) -> d.tidy.phrase.w.original
...
d.tidy.resp.counts.web <- d.tidy.resp.counts %>% group_by(np_match, specific) %>% mutate(mode = cleanedPhrase[which.max(n)])
```
...
df.mode <- left_join(d.tidy.phrase.w.original, d.tidy.resp.counts.web) %>% 
  subset(., !(np_match %in% c("day", "dusk", "night"))) %>%
...
write_csv(df.mode.final, '../data/pilot-classElicitation-free-4/class-elicitation-free-4-cleaned-data-update.csv')
