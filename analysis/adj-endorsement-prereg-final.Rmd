...
df.trials.filtered <- df.trials.first_submit %>%
  ...
  filter(trial_name == "adj_endorsement_both") %>%
  rowwise() %>%
  mutate(np_expectations =  item_cond)
...
df.trials.filtered <- df.trials.filtered %>% rowwise() %>%
  mutate(
    np_expectations = ifelse(is.na(np_expectations), df.mode.final %>% 
                               filter((superordinate_pl == superordinate) & (NP_sg == item)) %>% 
                               distinct(np_expectations) %>% .$np_expectations, np_expectations ))
df.trials.filtered <- df.trials.filtered %>% mutate(
