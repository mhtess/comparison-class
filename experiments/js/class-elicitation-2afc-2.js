...
function makeSlides(f) {
  ...
  function start() {
    ...
    exp.examples[i].super == "rooms" ?
      adjectivePhrase = adjectivePhrase + " in here" : null
    ...
    $('label[for=0]').html('"' + adjectivePhrase + " relative to other " + exp.examples[i][exp.sliderOrder[i][0]] + '."');
    $('label[for=1]').html('"' + adjectivePhrase + " relative to other " + exp.examples[i][exp.sliderOrder[i][1]] + '."');
  }
  ...
  function button() {
    ...
    if (!response) { $(".err").show(); }
      ...
      exp.data_trials.push({
        ...
        "paraphrase": exp.sliderOrder[i][response],
        "paraphrase0": exp.sliderOrder[i][0],
        "paraphrase1": exp.sliderOrder[i][1]
      });
      i++;
      exp.go();
    }
