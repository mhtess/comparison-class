...
author-information: >
  \author{{\large \bf Michael Henry Tessler$^1$} (mtessler@stanford.edu), {\large \bf Michael Lopez-Brau$^2$} (lopez\_mic@knights.ucf.edu) \\
...
abstract:
    "Speakers often refer only implicitly to context when using language.
...
If it's 75 \degree F (24 \degree C) outside, one might say "it's warm."
On the other hand, if it's 60 \degree F (16 \degree C), one might not consider it warm.
...
Comparison classes are necessary for understanding adjectives and, in fact, any use of language in which pragmatic reconstruction from context is critical, including vague quantifiers [e.g., "He ate a lot of burgers."; @SchollerFranke2015] and generic language [e.g., "Dogs are friendly"; @Tessler2016].
The problem for listeners is that the comparison class often goes unsaid (e.g., in "it's warm outside").
...
The existence of comparison classes in the study of vague language is uncontroversial [@Solt2009; @Bale2011]. 
4-year-olds categorize novel creatures (*pimwits*) as either *tall* or *short* depending on the distribution of heights of pimwits; because children's judgments are unaffected by the heights of creatures that aren't pimwits, the comparison class here is other pimwits [@Barner2008].
...
We propose that listeners actively combine category knowledge with knowledge about what classes are likely to be talked about to infer the reference class being used by the speaker.
...
For illustration, consider the weather. 
In Winter, the *positive* form of the adjective phrase "it's warm" should signal a specific class (i.e., relative to days in Winter) more so than the *negative* "it's cold" (which could signal a specific or general class, e.g., relative to the year). 
...
This is driven by the prior probability that the adjective could apply to the specific class, such as the chance that it's warm in Winter (Prediction 1).
In addition, regardless of the season, a listener should prefer "it's warm" to be relative to the current season, a more specific reference class. 
All else being equal, classes that are more specific will have relatively lower variance, which gives a vague utterance a more specific meaning (Prediction 2). 
...
The RSA model makes quantitative predictions given fine-grained details of the relevant prior knowledge: the distribution of temperatures for days in winter, in the whole year, and so on.
...
In Expt. 2 we collect judgements about sentences like "This [winter day] is cold relative to other days of the year," in which domain knowledge is needed but the comparison class is explicit.
We then use a Bayesian data analysis model to infer the parameters of the background knowledge from linguistic judgements.
<!--
As part of our (Bayesian) data analysis model we put uncertainty over the parameters of the background knowledge being used in our task.
This strategy has the virtue of avoiding asking participants numerical estimation questions, but it introduces a lot of uncertainty into the model.
We can reuse the RSA model to predict simple natural language data in the same domains (e.g., is it warm in Winter? Expt. 2), allowing us to estimate the model parameters reliably.
-->
...
<!-- by building more complete and coherent models of language, additional parameters can be estimated reliably by probing the model's linguistic intuitions  -->

<!-- to say different things using the same domain knowledge. -->


<!-- and use Bayesian data analysis to fill in the likely values. -->
...
The vagueness and context-sensitivity of these adjectival utterances can be modeled using a threshold semantics ($[[u]] = x > \theta$, for utterance $u$, scalar degree $x$, and threshold $\theta$), where the threshold comes from an uninformed prior distribution and is inferred in context via pragmatic reasoning [@Lassiter2013; see also @Qing2014a]:
\vspace{-0.5cm}
\begin{align}
...
She assumes the utterance came from an approximately rational Bayesian speaker $S_1$ trying to inform a naive listener $L_0$, who in turn updates her prior beliefs $P(x)$ via an utterance's literal meaning $[[u]](x)$.
...
The prior distribution over world-states is always relative to some comparison class $c$ (Eqs. \ref{eq:L1} \& \ref{eq:L0}).
...
$P(c)$ reflects listeners' expectations what classes are likely to be discussed.
...
Hence we fix each superordinate distribution to be a Unit-Normal $P(x \mid c_{super}) = \text{Gaussian}(0, 1)$ and the subordinate priors to also be Gaussian distributions $P(x \mid c_{sub}) = \text{Gaussian}(\mu_{sub}, \sigma_{sub})$; the subordinate priors thus have standardized units. 
...
<!-- \begin{tabular}{@{}lll@{}} -->
<!-- \toprule -->
<!-- Scale \{\emph{adjectives}\}                & Subordinate  classes                                       & Superordinate           \\ \midrule -->
...
<!-- Weight \{\emph{heavy}, \emph{light}\}    & \{watermelon, grape, apple\}                               & produce                 \\ \bottomrule -->
<!-- \end{tabular} -->
...
Figure \ref{fig:modelSchematics}, left, illustrates superordinate and possibel subordinate priors; e.g. temperatures over the whole year, in Winter (low), Fall (medium), and Summer (high), in a part of the world that exhibits seasons (e.g., Maryland, USA).
...
In sum, we see two predictions: The pragmatic listener overall prefers subordinate comparison classes, though the extent of this preference is modulated by the *a priori* probability that the adjective is true of the superordinate category (which depends on it's polarity).
...
This by itself, however, is not sufficient to fill in the probabilistic knowledge that the RSA model relies upon; putting uncertainty over the parameters of the prior knowledge in RSA would result in too many parameters to estimate reliably using the data from a single comparison class experiment.
-->
...
In particular, Expt. 2 we gather judgements about sentences with vague adjectuives but explicit comparison classes: whether or not an adjective would be true relative to the superordinate category of a subordinate member (e.g., it is a winter day, is it warm relative to days of the year?).


<!--
We choose an experiment that uses the same domain knowledge as used in the comparison class inference (Expt. 1; Table \ref{tab:1}) in order to limit the number of parameters in the model.
...
\ndg{yeah, there should be an expectation...}-->
...
Experimental paradigms, computational models, and data for this paper can be found at \url{https://mhtess.github.io}.
...
The materials and much of the design of the two experiments are the same and were preregistered:
\url{https://osf.io/hzdtj/}.
...
In this experiment, we gather human judgements of comparison classes in ambiguous contexts, testing the two predictions described in \textbf{Qualitative Model Predictions}.
...
In addition, we included in the model by-participant random effects of intercept and by-subordinate category random effects of intercept and iteraction between form and strength to predict the probability with which participants would choose the superordinate category as the paraphrase^[This was the maximal mixed-effects structure that converged.].
...
For items on the low end of the scale (e.g., temperature on Winter days), positive form adjectives are significantly more likely to lead *away* from superordinate comparison classes ($\beta = `r round(rs.glmm.simple.summary[["coefficients"]]["strength0:formpositive","Estimate"],2)`$;
...
This was followed by a question asking if the participant would endorse the positive- or negative-form adjective explicitly relative to the superordinate category (e.g., *Do you think the day in Winter would be warm relative to other days of the year?*).
<!-- Each participant saw 15 trials: one for each subordinate category paired with either the positive or negative form of its corresponding adjective. -->
...
Each subordinate category prior uses standardized units (given by the superordinate Unit-Normal), and we put the same priors over the parameters of each subordinate Gaussian: $\mu \sim \text{Uniform}(-3, 3)$, $\sigma \sim \text{Uniform}(0, 5)$.
...
We use priors consistent with previous literature: $\alpha_1 \sim \text{Uniform}(0, 20)$, $\alpha_2 \sim \text{Uniform}(0, 5)$
...
\ndg{this feels like an abrupt end... one more sentences glossing the results?}
...
This poses a challenge to listeners, who have to use context to uncover a more precise meaning to make sense of what they've heard.
...
This model made two novel predictions about what comparison classes listeners would prioritize.
It also made quantitative predictions about how background knowledge about the degree scale should translate into the probability with which a speaker will use a comparison class. 
Both of the qualitative and the quantitative predictions from the model were borne our in our first experiment. 
...
The prior may reflect basic level effects in categorization [@Rosch1975] or the hetereogeneity of the property within different classes.
For instance, the distribution over prices for kitchen appliances may be more hetereogenous than the distribution of heights for people, making "kitchen appliances" a relatively poorer reference class for the price of a specific kitchen appliance than "people" is for the height of a specific person.
Alternatively, the adjective phrase "It's warm outside" may in fact be an incomplete sentence, in a way analagous to sentence fragments studied in noisy-channel models of production and comprehension [@Bergen2015].
...
In previous work, we have attempted to measure prior knowledge by decomposing what would be implicitly multilayered estimation questions into multiple simpler questions, and then using a Bayesian data analytic model to reconstruct the prior knowledge [@Tessler2016; @Tessler2016cogsci].
We extend this approach to ask related *natural language* questions across two experiments to infer the parameters of the relevant prior knowledge.
...
Modeling language use thus yields productivity in experiment design, which can be harnessed to reduce uncertainty in the necessary parameters of language and cognitive models.
