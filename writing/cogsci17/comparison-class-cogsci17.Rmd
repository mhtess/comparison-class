...
df.scaledFreq <- left_join(df.freq.long %>%
  ...
  mutate(logval = log(val),
         scaledLogval = 0.2*logval,
         scaledVal = exp(scaledLogval)) %>%
  select(-logval, -scaledLogval, -val) %>%
  spread(key, scaledVal) %>%
  mutate(sumScaledFreq = freq + superFreq,
         cPrior = freq / sumScaledFreq)
```
...
```{r expt1results, fig.env = "figure*", fig.pos = "htb", fig.width=6.6, fig.height=4.8, fig.align='center', set.cap.width=T, num.cols.cap=2, fig.cap = "Empirical data, model inferred world priors, and empirically measured comparison class priors.. Top: Experiment 1 results. Comparison class judgments in terms of proportion judgments in favor of superordinate comparison class. Error bars correspond to bootstrapped 95 percent confidence intervals. Middle: Inferred prior distributions of world knowledge used in Experiments 1 and 2. Bottom: Google WebGram scraped comparison class priors."}
...
fig2b <- ggplot(m.samp.tidy %>% mutate(experiment = "Inferred priors"), 
                aes(x = val, y = ..scaled.., 
...
fig2c <- ggplot(
  ...
  aes( x = cat, y = 1- cPrior, fill = category))+
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 0.95),
        axis.title.x = element_blank()) +
  ylim(0, 1) + 
  ylab("Prior superordinate class")
...
-->
```{r posteriorPredictive_scatters, fig.env = "figure*", fig.pos = "htb", fig.width=6.6, fig.height=2.5, fig.align='center', set.cap.width=T, num.cols.cap=2, fig.cap = "caption"}
...
(1) Listeners should prefer comparison classes that are relatively lower variance as they result in more specific meanings and 
(2) The comparison class will shift with the *a priori* probability of the adjective being true of the lower variance class.
...
These probabilities likely conflate multiple cognitive factors such as the basic level effects in categorization [@Rosch1975] or the hetereogeneity of different classes (e.g., the distribution over prices of kitchen appliances may be more hetereogenous than the distribution of heights for people).
...
It seems that listeners can jointly infer a lot from a little: The context and the meaning from a single utterance.
...
We got people to thank. 
We got monies to acknowledge.
