...
prior.samples <- webppl(priorModel, data = prior.params, data_var = "priorParamsFromR")

prior.tidy <- prior.samples %>% 
  rename(subordinate = value.subcat, superordinate = value.supercat) %>%
  gather(key, val)

ggplot(prior.tidy, aes(x = val, y=..scaled.., fill = key))+
...
```{r echo = F, results = "asis", fig.env = "table*", fig.pos = "h", fig.width = 4, fig.height = 2, fig.align = "center", set.cap.width = T, num.cols.cap = 2, fig.cap = "This image spans both columns. And the caption text is limited to 0.8 of the width of the document."}
