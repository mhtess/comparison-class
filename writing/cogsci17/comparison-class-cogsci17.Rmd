...
abstract:
    ...
    Quantative predictions of the model rely upon the structure of relevant world knowledge, which we elicit using a novel technique that avoids participants having to esimate quantities.
    "
...
@Salt2012 show how what counts as "dark" or "tall" depends upon the statistics of the comparison class.
...
This question is not just an issue for gradable adjectives but in fact for any vague language that can be modeled using a threshold semantics [e.g., generic language, see @Tessler2016]
...
2 participants were excluded for failing the catch trial, leaving a total number of 262 participants.
...
We tested our hypothesis using positive and negative form gradable adjectives describing 5 scales: price (*expensive*, *cheap*), temperature (*warm*, *cold*), duration (*long*, *short*), height (*tall*, *short*), and weight (*heavy*, *light*).
Each scale was paired with a superordinate category: kitchen appliances (price), weather (temperature), things you watch online (duration), people (height), produce (weight).
For each superordinate category, we used 3 subordinate categories that aimed to be situated near the high-end (e.g., *basketball player*), low-end (e.g., *gymnast*), and intermediate (e.g., *soccer player*) of the degree scale (here, height).
This resulted in 30 unique events (\{3 subordinate\} x \{5 scales\} x \{2 adjective forms (positive, negative)\}; see Table 1 for full listing).
...
"The music video is short relative to music videos" (subordinate comparison class) or "The music video is short relative to things you watch online" (superordinate comparison class). 
