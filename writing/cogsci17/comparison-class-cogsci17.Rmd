...
When it's warm outside it's still useful to know whether it's July or January
A warm day in July could be 80 degrees while a warm day in January might not above 60.
The predicate "warm" is vague, in the sense that there are borderline cases of being warm, interlocutors can faultlessly disagree as to whether or not it is warm, and what it means to be warm is relative to a (often contextually-specified) relevant *comparison class*.

Comparison classes are uncontroversial in the study of vague language. 
Even, 2 or 3 year olds understand that the words "big" and "little" depend upon the context: 
An object can be "big" relative to other objects in a scene (i.e., the physical context), 
...
Adult judgments of the felicity for gradable adjectives like "dark" or "tall" in adults depend upon the statistics of the comparison class [@Solt2012].
What is less well understood is how a comparison class becomes contextually-salient. 
...
We introduce a novel language understanding model that can resolve uncertainty about the likely reference class alongside the uncertainty know to be important for modeling vague language understanding (namely, uncertainty about the world and uncertainty about the precise meaning of the speech act).
...
Instead, we build a Bayesian data analysis model to put uncertainty over these paremters and resolve the uncertainty by asking further natural language questions to our language understanding model. 
...
Adjectives like *warm* and *cold*, *cheap* and *expensive*,  are vague descriptions of an underlying quantitative scale (e.g., price, temperature).
...
A day in Winter is warm *relative to* other days in Winter and probably not relative to all days of the year.

The comparison class cam be thought of as a probability distribution over degrees on a scale [@Lassiter2013; @Qing2014].
A probabilistic model of pragmatic reasoning [e.g., the Rational Speech Act model; @Goodman2016] can use a standard threshold semantics and reason about the comparison class to arrive at context-sensitive adjective interpretation.
...
\begin{tabular}{lll}
  \hline
Scale \{Adjectives\}& Subordinate & Superordinate \\
  \hline
Height \{ \emph{tall}, \emph{short}\} & \{professional basketball player,  professional soccer player, professional gymnast\} & people \\
  Price \{ \emph{expensive}, \emph{cheap}\}& \{dishwasher, toaster, oven\} & kitchen appliances \\
  Temperature \{\emph{warm}, \emph{cold}\}& \{ Summer, Fall, Winter\} in Maryland & days of the year \\
...
This resulted in 30 unique events (\{3 subordinate categories\} x \{5 scales\} x \{2 adjective forms\}; see Table 1 for full listing).
...
Participants were asked a question of the form "What do you think Gary meant?" and given two options to rephrase the adjective sentence with an explicit comparison class of either the subordinate or superordinate category: \mlb{change the font to match "DO ACTION"/"DONE ACTION" text in habituals paper}
\begin{itemize}
\item The SUBORDINATE CATEGORY is ADJECTIVE relative to SUBORDINATE CATEGORY (e.g., \textit{The music video is short relative to music videos})
\item The SUBORDINATE CATEGORY is ADJECTIVE relative to SUPERORDINATE CATEGORY (e.g., \textit{The music video is short relative to things you watch online})
\end{itemize}
\noindent The experiment can be viewed in full at http://stanford.edu/~mtessler/comparison-class/experiments/class-elicitation-2afc.html.
...
In this experiment, we would like to elicit the priors over the subordinate categories.
We use a two-alternative forced choice paradigm to collect peoples' prior belief on the properties of certain items.
...
The experiment can be viewed in full at \url{http://stanford.edu/~mtessler/comparison-class/experiments/vague-prior-elicitation-2afc.html}.
