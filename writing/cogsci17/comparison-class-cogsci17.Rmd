...
You might not say it's warm if it's 60\degree F (16\degree C).
That is, unless it's January, in which case, it could be warm for January.
*Warm* is vague and relative, and its felicitous application depends upon what the speaker is using as a basis of comparison --- the *comparison class*, (e.g., other days in the year or other days in Winter).
As in the utterance "It's warm outside", this comparison class often goes unsaid.
<!--
...
A day in Winter is also a day of the year, and so if it's warm, it could be *warm for Winter* or *warm relative to other days of the year*. 
...
We propose that listeners actively use knowledge about possible classes and knowledge about what classes are likely to be talked about to infer what reference class is being used by the speaker.
...
(2) The likely comparison class will be modulated by the *a priori* plausibility that the adjective could apply to a member of specific class relative to the general class (e.g., how likely a day in Winter is to be warm relative to other days of the year). 
This second prediction in turn depends upon the central tendencies of the specific vs. general classes (e.g., is the specific class expected to be high or low relative to the general class?) and the form the adjective (positive or negative form, e.g., warm or cold). 
...
The RSA model makes quantitative predictions about the comparison class, but these rely upon the fine-grained details of the relevant world knowledge.
This can be measured by having participants estimate quantities or give likelihood judgments in the same contexts without experimental manipulation [@FrankeEtAl2016].
Instead, we build a Bayesian data-analytic model to introduce uncertainty over the parameters of this world knowledge.
This data-analytic model has too many parameters to estimate reliably from the Expt. 1 data alone.
We use our same RSA model to predict data for similar kinds of language tasks in a follow-up experiment; critically, the same model with the same prior knowledge is used to predict data from both experiments.
...
The vagueness and context-sensitivity of these adjectives can be modeled using a threshold semantics ($[[u]] = x > \theta$) where the threshold $\theta$ comes from an uninformative prior distribution and is inferred in context via pragmatic reasoning [@Lassiter2013; see also @Qing2014a].
<!---->
\begin{align}
...
She assumes the utterance came from a soft-max rational Bayesian speaker $S_1$ who tried to inform a naive listener $L_0$, who in turn updates her prior beliefs $P(x)$ via an utterance's literal meaning $[[u]](x)$.
The model of @Lassiter2013 introduced uncertainty over a semantic variable, the truth-functional threshold $\theta$ (Eq. \ref{eq:L1}).
The uncertainty over $\theta$ (e.g., the point at which something is *warm*) interacts with the prior distribution over possible states of the world $P_{c}(x)$ (e.g., possible temperatures) and recursive pragmatic reasoning to resolve the meaning of the adjective in context.
...
She does this using her world knowledge of what worlds are plausible given different comparison classes $P(x \mid c)$, what comparison classes are likely to be talked about $P(c)$, and what a rational speaker would say in given a world and comparison class $S_{1}(u \mid x, c, \theta)$.
...
We elaborate the Rational Speech Act theory for adjective understanding [@Lassiter2013; @Lassiter2015] by introducing uncertainty over the class of entities against which the target entity is compared --- the comparison class $c$ (Eq. \ref{eq:L1a}).
As a first test of this idea, we consider an idealized case where the comparison class can either be a relatively specific or relatively general categorization (e.g., warm relative to days in Winter or relative to days of the year). 
The listener is aware that the target entity is a member of the subordinate class (e.g., aware that it is winter) and draws likely values of the degree (e.g., temperature) from the subordinate class prior: $P(x \mid c_{sub})$.
<!---->
\begin{align}
...
\end{align}
<!---->
We use a speaker model that considers the alternatives of using the same utterance with the explicit comparison class (e.g., "It is warm relative to other days in Winter.", "It is warm relative to other days of the year.") as well as the alternative of remaining silent (a null utterance with no informational content). 
\mht{<-- modify depending on what the final set of alternatives is}
