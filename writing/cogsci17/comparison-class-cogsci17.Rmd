...
fig2a <- df.summary %>%
  ungroup() %>%
  mutate(experiment = 'Comparison class data',
...
fig2b <- ggplot(m.samp.tidy %>% mutate(experiment = "Degree priors"), 
                aes(x = val, y = ..scaled.., 
...
fig2c <- ggplot(
  ...
  experiment  = "Comparison class prior"), 
...
$P(x \mid c)$ describes the plausible values of a degree $x$ (e.g., temperature) given a comparison class $c$ (e.g., days in Winter).
...
This by itself will not be sufficient to fill in the probabilistic knowledge that our RSA model relies upon; the model as such would be overparameterized.
...
\end{center}
\caption{Full RSA - data analysis model. The recursive Bayesian RSA model is visualized in the center block. $L_1$ is used as a model for Experiment 1 data and  $S_2$ for Experiment 2 data. The subordinate category priors $P(x \mid c_{sub})$ are used in both experiments and are modeled as Gaussians with unknown mean and variance. (See main text for priors.) Note that RSA cannot be visualizing using standard graphical model notation as it's a recursive model.}
...
Both of these considerations lead us to design a second experiment.

We choose an experiment that uses the same domain knowledge as Expt. 1 in order restrict the number of parameters in the model.
...
The goal of this analysis is use data from both experiments to simulatenously reconstruct the prior knowledge used in the tasks and generate predictions for the two data sets.
We use Eq. \ref{eq:L1a} to model the comparison class inference task (Expt. 1).
We add a level of recursion to the model and use a pragmatic speaker to model the adjective production data [Expt. 2; following @Qing2014a; @Tessler2016; @Tessler2016cogsci].
...
```{r posteriorPredictive_scatters, fig.env = "figure*", fig.pos = "htb", fig.width=6.6, fig.height=4, fig.align='center', set.cap.width=T, num.cols.cap=2, fig.cap = "Human endorsement of comparison class paraphrases (middle) and adjective sentences (right) as a function of listener model $L_{1}$ and speaker model $S_{2}$ predictions, respectively. The left facet displays a subset of the paraphrase data (Expt. 1) to reveal good quantitative fit even in a small dynamic range. Error bars correspond to 95\\% bootstrapped confidence intervals for the participant data and 95\\% highest probability density intervals for the model predictions."}
scatterSubFig <- ggplot(md.pp %>% filter(mean < 0.30 & expt == "Comparison class inference") %>%
                          mutate(expt = factor(expt, labels = c("Comparison class inference (subset)"))), 
...
grid.arrange(scatterSubFig, scatterFig, nrow = 1, widths = 1.15:2.5)
```
...
The second contribution of this paper is a novel data-analytic approach, where prior knowledge used in the Bayesian model is reconstructed from converging evidence gathered from experiments that use similar language about the same domains.
