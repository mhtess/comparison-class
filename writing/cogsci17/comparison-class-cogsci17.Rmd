...
abstract:
    ...
    \ndg{the abstract is too... abstract. add a concrete example. make it interpretable to average cogsci person, not just a semanticist.}
    "
...
```{r expt1results, fig.env = "figure*", fig.pos = "htb", fig.width=6.6, fig.height=2.5, fig.align='center', set.cap.width=T, num.cols.cap=2, fig.cap = "Experiment 1 results. Comparison class judgments for 30 items on 5 scales. Error bars correspond to bootstrapped 95 percent confidence intervals."}
...
$P(x \mid c)$ describes the likely values of a degree (e.g., temperature) $x$ given a comparison class $c$ (e.g., days in Winter).
...
The model also has two parameters not of direct theoretical interest: the speaker optimality parameters $\alpha$ and the cost $w$
We put the following priors over these parameters: $\alpha \sim \text{Uniform}(0, 20)$, $w \sim \text{Uniform} (0, 20)$
\mht{describe task-wise speaker optimalities}
