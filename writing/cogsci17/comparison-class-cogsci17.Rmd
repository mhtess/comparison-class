...
abstract:
    ...
    *Warm* conveys that the temperature is high relative to some comparison class, but little is known about how a listener decides upon such a standard of comparison. 
    ...
    We introduce a Rational Speech Act model and derive two novel predictions from the model, which we validate using a paraphrase experiment to measure listeners' beliefs about the likely comparison class.
    ...
    This represents a new possibility for Bayesian models of cognition and language: by building more complete and coherent models of language, additional parameters can be estimated reliably by asking the model to answer multiple questions using the same domain knowledge.
    "
...
You might say "It's warm outside" if it's 75\degree F (24\degree C).
You probably wouldn't say it if it's 60\degree F (16\degree C), unless it's January, in which case, it could be warm for January.
*Warm* is vague and the accurate application of the adjective depends upon the speaker's *comparison class*, (e.g., other days in January, or something more general like all others days in the year).
This comparison class often goes unsaid.
<!--
...
Comparison classes are uncontroversial in the study of vague language [@Solt2009; @Bale2011]. 
4 year-olds categorize novel creatures (*pimwits*) as either *tall* or *short* depending on the distribution of heights of pimwits and, critically, not the heights of creatures that aren't pimwits [@Barner2008].
Adult judgments of the felicity for gradable adjectives like "dark" or "light", "short" or "tall" similarly depend upon fine-grained details of the statistics of the comparison class [@Schmidt2009; @Solt2012; @Qing2014].
...
We propose that listeners actively infer what reference class is being used by the speaker.
...
(1) *Ceteris paribus*, a listener should prefer reference classes that are more specific because lower variance in the reference class will provide a more specific meaning to a vague utterance. 
(2) The likely comparison class will be modulated by the *a priori* plausibility that the adjective could apply (relative to the general class) to a member of specific class. 
This second prediction in turn depends upon where on the degree scale the class falls (e.g., high or low) and the form the adjective (positive or negative form). 
...
Instead, we build a Bayesian data-analytic model to introduce uncertainty over these parameters.
With only the data from Expt. 1, the data-analytic model has too many parameters to estimate reliably.
We can use our same RSA model to generate predictions for other similar kinds of language in a follow-up experiment; critically, the same model with the same prior knowledge will be used to predict data from both experiments.
This represents a new possibility for Bayesian models of cognition and language: by building more complete and coherent models of language understanding, additional parameters can be estimated reliably by asking the model to answer multiple questions about the same domain knowledge.
...
The vagueness and context-sensitivity of these adjectives can be modeled using a threshold semantics where the threshold comes from a uniform probability distriution and is inferred in context via pragmatic reasoning [@Lassiter2013; see also @Qing2014a].

\begin{align}
L_{1}(x, \theta \mid u) &\propto S_{1}(u \mid x, \theta) \cdot P_{cc}(x) \cdot P(\theta) \label{eq:L1} \\
S_{1}(u \mid x, \theta) &\propto \exp{(\alpha_1 \cdot \ln {L_{0}(x \mid u, \theta)})} \label{eq:S1}\\
L_{0}(x \mid u, \theta) &\propto {\delta_{[[u]](x, \theta)} \cdot P_{cc}(x)}. \label{eq:L0}
\end{align}
...
In this framework, the pragmatic listener $L_1$ tries to resolve the state of the world $x$ from the utterance she heard $u$.
She assumes the utterance came from a soft-max rational Bayesian speaker $S_1$ who tried to coordainte with a naive listener $L_0$ who updates her prior beliefs $P(x)$ via an utterance's literal meaning $[[u]](x)$
The model of @Lassiter2013 introduced to RSA a "lifted threshold" $\theta$ (so-called "lifted", because there is uncertainty at the level of the pragmatic listener $L_1$, but not at the lower levels of recursion $S_1$ and $L_0$).
The uncertainty over the semantic threshold $\theta$ (i.e., the point at which something is *warm*) interacts with the prior distribution over possible states of the world (e.g., possible temperatures) $P_{cc}(x)$ and recursive pragmatic reasoning to resolve the meaning of the adjective in context.
What is made clear from this model is that the prior distribution over world-states is always relative to some comparison class $cc$ (Eqs. \ref{eq:L1},\ref{eq:L0}).
But where does such a comparison class come from?

When a listener hears only that "It's warm outside" without an explicit comparison class (e.g., "warm for the season."), we imagine that a listener must infer the comparison class by thinking about what would best complete the sentence, in a way analagous to noisy-channel models of production and comprehension [@Bergen2015].
She does this using her world knowledge of what worlds are plausible given different comparison classes---$P(x \mid cc)$, what comparison classes are likely to be talked about---$P(cc)$, and what a rational speaker would say given a world and comparison class $S_{1}(u \mid x, cc, \theta)$.


<!--
Contemporary linguistic theories posit that the meaning of such words are simple thresholds on the measure [@Kennedy2007].
...
Such a representation can be coupled with an threshold semantics (i.e., *warm* means greater than some temperature: $[[u]]: x > \theta$) that has uncertainty over the threshold ($\theta \sim \text{Uniform}(0, 1)$) to derive comparison class specific interpretations of gradable adjectives like *warm*.
...
How does a listener settle on the appropriate comparison class, if none is specifically articulated by the speaker?
-->
<!--
The model is an extension of the Rational Speech Act theory of language understanding [for a review see: @Goodman2016] and uses a simple threshold semantics for adjectives  (i.e., warm means greater than some temperature).
...
We elaborate the Rational Speech Act theory for adjective understanding [@Lassiter2013; @Lassiter2015] by introducing uncertainty over the class of entities against which the target entity is compared --- the comparison class $cc$.
As a first test of this idea, we consider an idealized case where the comparison class can either be a relatively specific or relatively general categorization (e.g., wrarm relative to days in Winter, or relative to days of the year in general). 
The listener is aware that the target entity is a member of the subordinate class (e.g., aware that it is winter) and draws likely values of the degree (e.g., temperature) from the subordinate class prior: $P(x \mid cc_{sub})$.
<!---->
\begin{align}
L_{1}(x, cc, \theta \mid u) &\propto S_{1}(u \mid x, cc, \theta) \cdot P(x \mid cc_{sub}) \cdot P(cc) \cdot P(\theta) \label{eq:L1a}\\
S_{1}(u \mid x, cc, \theta) &\propto \exp{(\alpha_1 \cdot \ln {L_{0}(x \mid u, cc, \theta)})} \label{eq:S1a}\\
L_{0}(x \mid u, cc, \theta) &\propto {\delta_{[[u]](x, \theta)} \cdot P(x \mid cc)}. \label{eq:L0a}
\end{align}
<!---->
...
For purposes of illustration, we assume each class is equally likely *a priori*^[We will later relax this idealization with an empirically derived prior on comparison classes.]: $c \sim \text{UniformDraw}([c_{sub}, c_{super}])$.
We also assume the listener $L_1$ knows the target entity is a member of the subordinate class (e.g., knows that it's Winter): $P(x \mid c_{sub})$, but that the speaker might have had a different comparison class in mind.
...
We see that regardless of the adjective form (positive or negative) or the mean of the prior distribution (low, medium, high), the pragmatic listener prefers the more specific comparison class (i.e., the subordinate class; Figure \ref{fig:modelSchematics}, right; all bars below baseline).
...
We also see that when the pragmatic listener (Eq. \ref{eq:L1}) is in Winter and hears that "It's warm" (positive adjective), she thinks it's more likely that it's warm relative to days in Winter than when she hear's it's cold (Figure \ref{fig:modelSchematics}, right, left-most bars).
...
The RSA model makes quantitative predictions as well, though these will depend on the quantitative details of the listener's world knowledge: $P(x \mid c)$ in Eqs. \ref{eq:L1a} \& \ref{eq:L0a}.
This kind of world knowledge can be measured by having participants estimate certain quantities without any experimental manipulation [e.g., the temperature in Maryland on a day in Winter; @FrankeEtAl2016].
...
Here, we extend this approach to ask binary judgment *natural language* questions that use the same prior knowledge as would be relevant for Expt.~1 and which the same RSA model can answer.

# Full model and Experiment 2

The pragmatic listener model in Eq. \ref{eq:L1a} is a model of comparison class inference (Expt. 1).
In order to make quantitative predictions, we must specify the listener's world knowledge $P(x \mid c_{sub})$ and $P(x \mid c_{super})$ as well as the prior distribution on comparison classes $P(c)$.
...
], and scale it with a free parameter $\beta$: $P(c) \propto \exp{(\beta \cdot \log \hat{f})}$
\mht{Should I include the scraped frequencies in the report? or as a supplement on Rpubs?}
In our data analysis, we use the following prior: $\beta \sim \text{Uniform}(0, 3)$.


### Inferring priors on world knowledge

$P(x \mid c)$ describes the plausible values of a degree (e.g., temperature) $x$ given a comparison class $c$ (e.g., days in Winter).
Rather than measuring these distributions, we will make a few assumptions about their functional forms and infer their parameters using Bayesian data analysis.
This by itself will not be sufficient to fill in the probabilistic knowledge into our model, as the model will be overparameterized.
We need not worry though; the RSA model is a productive language model, and as such, we can ask it other questions to gain more traction on the latent world knowledge.

For simplicity, we assume each $P(x \mid c)$ is a Gaussian distribution. 
We fix each superordinate distribution $P(x \mid c_{super})$ to be a Unit-Normal and put priors over the parameters of each subordinate Gaussian. 
Since each domain is using standardized units (given by the superordinate Unit-Normal), we can put the same priors over the parameters of each subordinate Gaussian: $\mu \sim \text{Uniform}(-3, 3)$, $\sigma \sim \text{Uniform}(0, 5)$.


### Predicting more data

Currently, the full model has 2 parameters for each subordinate category (the $\mu$ and $\sigma$ of $P(x \mid c_{sub})$), in addition to weight $\beta$ on the empirical ngram frequency for $P(c)$.
These are too many parameters to estimate reliably using only the data from Expt. 1. 
...
Both of these considerations lead us to read a second experiment.

We choose an experiment that uses the same domain knowledge as Expt. 1 in order to not add more parameters.
In addition to helping us gain traction on participants' prior knowledge, a second experiment can further test the RSA model. 
...
Since we have a language understanding model that can make predictions about adjective use, we re-use this model to predict participants' endorsements of adjectives.
...
We use Eq.~\ref{eq:L1a} to model the comparison class inference task of Experiment 1.
The model the adjective production data in Experiment 2, we add a level of recursion to use a pragmatic speaker [following: @Qing2014a; @Tessler2016; @Tessler2016cogsci].
We also remove comparison class uncertainty, since the experiment make use of explicit comparison classes.
<!---->
\begin{align}
...
We use Eq.~\ref{eq:S2} to model the data from Expt. 2 (Figure \ref{fig:modelDiagram}).

The models for the two tasks make use of the same prior knowledge $P(x \mid c)$. 
We can thus infer the likely values of the parameters of these priors from the two experiments worth of data, and use those inferred parameters and make predictions about what the data should look like. 

The full model also has two parameters not of direct theoretical interest: the speaker optimality parameters $\alpha^\text{expt}_{i}$, which likely vary across the two tasks.
...
<!--
Human endorsements of comparison class paraphrases (left) and adjective sentences (right) as a function of listener model and speaker model predictions, respectively. Errorbars correspond to 95 percent bootstrapped confidence intervals for the participant data and 95 percent highest probability density intervals for the model predictions.
-->
```{r posteriorPredictive_scatters, fig.env = "figure*", fig.pos = "htb", fig.width=6.6, fig.height=3, fig.align='center', set.cap.width=T, num.cols.cap=2, fig.cap = "caption"}
...
md.pp <- left_join(
  ...
  mutate(expt = factor(expt, levels = c( "superCC", "superSpeaker"),
                       labels = c('Comparison class inference', 'Adjective production')))
...
expt1.r2 <- round(with(md.pp %>%
       filter(expt == "Comparison class inference"), cor(MAP, mean))^2, 3)

expt1.n <- length(filter(md.pp, expt == "Comparison class inference")[["MAP"]])

expt2.r2 <- round(with(md.pp %>%
       filter(expt == "Adjective production"), cor(MAP, mean))^2, 3)

expt2.n <- length(filter(md.pp, expt == "Adjective production")[["MAP"]])
```

As shown in Figure \ref{fig:posteriorPredictive_scatters}, the model does a good job at reconstructing the variability in responses for Experiment 1: $r^2(`r expt1.n`) = `r expt1.r2`$, and Experiment 2: $r^2(`r expt2.n`) = `r expt2.r2`$.
...
$\alpha^{1}_{1} = `r a1.1.map` [`r a1.1.lower`, `r a1.1.upper`]$;
$\beta = `r b.map` [`r b.lower`, `r b.upper`]$ 

Model parameters specific to the $S_2$ model used for Experiment 2: 
$\alpha^{2}_{1} = `r a1.2.map` [`r a1.2.lower`, `r a1.2.upper`]$;
$\alpha^{2}_{2} = `r a2.2.map` [`r a2.2.lower`, `r a2.2.upper`]$
...
Vagueness in language (e.g., saying "It's warm outside") poses a challenge to listeners, who have to use context to uncover a more precise meaning (e.g, a rough estimate of the temperature) to make sense of what they've heard.
Context, however, can also be underspecified, as there are many possible dimensions or categories that the speaker might be implicitly comparing against.
Here, we investigate the flexibility in the reference class against which an entity can be implicitly compared.
We derive a language understanding model that can flexibly reason about the likely comparison class.
This model made two qualitative predictions: 
(1) Listeners should prefer comparison classes that are relatively more specific (i.e., lower variance) as they result in more specific meanings and 
(2) When the target entity (e.g., the temperature of the day) is towards the upper part of the scale (e.g., during Summer), a positive form adjective (e.g., warm) should lead listeners to infer a more general comparison class than a negative form adjective (e.g., cold). The opposite is predicted to be true of entities towards the lower end of the scale (e.g., days in Winter).
...
The second contribution of this paper is a novel data-analytic approach, where prior knowledge used in the Bayesian model is reconstructed from converging evidence gathered from an experiment that uses similar language about the same domains.
The major feature of this approach is that participants may respond only to simple natural language questions rather than estimating numerical quantities for which complicated linking functions must be designed [@FrankeEtAl2016].
It also provides a further validation of the language model, which must predict data from two similar but distinct language experiments. 
...
We posited that the phrase "It's warm outside" is an incomplete sentence requiring a completion analagous to those investigated with noisy-channel models of production and comprehension [e.g., @Bergen2016].
...
There are more cognitive interpretations of the comparison class prior, perhaps relating to basic level effects in categorization [@Rosch].
\mht{Also, heterogeneity of the class w.r.t. dimension, e.g. kitchen appliances}
It is hard to disentangle these two possibilities in this paradigm, as they are both correlated with frequency.
