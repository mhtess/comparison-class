...
abstract:
    ...
    *Warm* conveys that the temperature is high relative to some comparison class, but little is known about how a listener decides upon a comparison class. 
    ...
    We generate quantitative model predictionsby asking follow-up questions to participants and the model, combined with a novel Bayesian data analytic technique. 
    We find that the listeners are remarkably flexible in their ability to infer speakers' intended reference classes when they are absent from the context."
...
When you read this, it may or may not be *warm* outside.
If it's 60\degree F (16\degree C), that's probably not warm.
That is, unless it's January, in which case, that could be warm for January.
People can disagree as to whether or not it is warm since what it means to be warm is relative to a *comparison class*, (e.g., other days in January, or something more general like all others days in year) and this comparison class often goes unsaid.
<!--
...
4 year-olds categorize novel creatures (called *pimwits*) as either *tall* or *short* depending on the distribution of heights of pimwits and, critically, not the heights of creatures that aren't pimwits [@Barner2008].
...
We propose that listeners must actively infer what reference class is being used by the speaker.
...
We introduce a novel language understanding model that can flexibly reason about the likely reference with pragmatic reasoning.
...
(2) The likely comparison class will be driven by the *a priori* plausibility of the adjective to the specific class, which in turn depends upon where on the degree scale the class falls (e.g., high or low) and the form the adjective (positive or negative form). 
...
This data-analytic model has too many parameters to estimate reliably from the results of just the first experiment. 
We can use our same RSA model to generate predictions for other similar kinds of language in a follow-up experiment, one that relies upon the same prior knowledge used in Expt.~1.
This represents a new possibility for Bayesian models of cognition and language: by building more complete and coherent models of language, additional parameters can be estimated reliably by asking the model to answer multiple questions about the same domain knowledge.
...
The vagueness and context-sensitivity of these adjectives can be modeled using a threshold semantics where the threshold is left underspecified and is inferred in context via pragmatic reasoning [@Lassiter2013; see also @Qing2014a].
...
This is a Rational Speech Act (RSA) model, a recursive Bayesian model where speaker $S$ and listener $L$ coordinate on an intended meaning [for a review of this modeling framework, see @Goodman2016].
The model of @Lassiter2013 introduced to RSA a "lifted threshold" $\theta$ (so-called "lifted", because it there is uncertainty at the level of the pragmatic listener $L_1$, but not at the lower levels of recursion $S_1$ and $L_0$).
By putting a prior distribution over $\theta$, the semantic threshold for an adjective (like *warm*), the prior distribution over possible states of the world (e.g., possible temperatures) $P_{cc}(x)$ and recursive pragmatic reasoning are used to resolve the context-specific meaning to the adjective.
...
When a listener hears only that "It's warm outside" without an explicit comparison class (e.g., "...relative to other days in Winter."), we imagine that a listener must infer the comparison class by imagining the likely comparison class to complete the sentence in a way analagous to noisy-channel models of production and comprehension [@Bergen2016].
She does this using her world knowledge of what temperatures are plausible given different comparison classes---$P(x \mid cc)$, what comparison classes are likely to be talked about---$P(cc)$, and how a rational speaker would choose utterances.
...
In addition to the uncertainty over the comparison class $cc$, a pragmatic listener also has uncertainty over the value of the degree $x$ (e.g., the temperature of the day) and the semantic threshold variable $\theta$ as in @Lassiter2013.
Uncertainty about the value of the degree $x$ is always with respect to a comparison class $cc$: $P(x \mid cc)$.

As a first test of this idea, we consider an idealized case where the comparison class can either be a relatively specific or relatively general categorization (e.g., relative to days in Winter or relative to days of the year in general). 
The listener is aware that the target entity is a member of the subordinate class (e.g., aware that it is winter) and draws likely values of the degree from the subordinate class prior: $P(x \mid cc_{sub}$.
<!---->
\begin{align}
L_{1}(x, cc, \theta \mid u) &\propto S_{1}(u \mid x, cc, \theta) \cdot P(x \mid cc_{sub}) \cdot P(cc) \cdot P(\theta) \label{eq:L1a}
\end{align}
<!---->
The listener believes the speaker had some intended comparison class in mind, as well as some degree he was trying to communicate and some threshold he was using: $S_{1}(u \mid x, cc, \theta)$.
