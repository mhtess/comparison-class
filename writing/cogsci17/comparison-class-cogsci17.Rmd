...
model.path <- "/media/michael/Data/Desktop/comparison-class/models/"
...
In this experiment, we elicit the comparison class \textit{c} for different events in order to test our model predictions. We use a two-alternative forced choice paradigm to collect peoples' endorsements of a subordinate or superordinate comparison class.
...
We recruited 300 participants from Amazon Mechanical Turk. Two participants were excluded due to failing the catch trial. Participation was restricted to those with U.S. IP addresses and who had at least a 95% work approval rating. On average, the experiment took five minutes and participants were compensated PAYMENT for their work. (Include simulation bit here with n = 300?)
...
We created 30 events using positive- and negative-form gradable adjectives from five different measureable scales, each conditioned with three context sentences. The scales from which we derived these adjectives were height (e.g., \textit{tall basketball player}, \textit{short gymnast}), price (e.g., expensive toaster, cheap oven), temperature (e.g., \textit{warm day in Summer}, \textit{cold day in Winter}) time (e.g., \textit{long music video}, \textit{short movie}), and weight (e.g., \textit{light grape}, \textit{heavy watermelon}). Within each context sentence, an item was introduced. The items served as their own subordinate categories and the item triplets assigned to each scale (e.g., \textit{oven}, \textit{toaster}, \textit{dishwasher} in the scale price}) shared a common supercategory (e.g., \textit{kitchen appliances}).
...
For each event, participants were given a context sentence to introduce the item (subordinate category), followed by a target sentence, which predicated a positive- or negative-form gradable adjective over the item. The participant was then asked if they believed that the target sentence was true of the subordinate or superordinate category. For example, one event read: "Gary finds a music video to watch online with his friend." "Gary says to his friend, "It's short."" "What do you think Gary meant?" The participant is then asked if they think that Gary meant that the music video was long relative to music videos (subordinate comparison class) or long relative to videos online (superordinate comparison class). Every participant saw a randomized and unique set of events and names. The experiment can be viewed in full at http://stanford.edu/~mtessler/comparison-class/experiments/class-elicitation-2afc.html.

## Results

# Experiment 2: Vague speakers

In this experiment, we would like to elicit the priors over comparison
...
We created 30 events using positive- and negative-form gradable adjectives from five different measureable scales, each conditioned with three context sentences. The scales we derived adjectives from were height (e.g., \textit{tall basketball player}, \textit{short gymnast}), price (e.g., expensive toaster, cheap oven), temperature (e.g., \textit{warm day in Summer}, \textit{cold day in Winter}) time (e.g., \textit{long music video}, \textit{short movie}), and weight (e.g., \textit{light grape}, \textit{heavy watermelon}). Three items were assigned to each scale, where each item was its own subordinate category and all had a common superordinate category.

### Procedure

For each event, participants saw a context sentence, 
