...
abstract:
    "Speakers are not always explicit when they use vague language.
...
*Warm* is relative, and its felicitous application depends upon what the speaker is using as a basis of comparison --- the *comparison class* (e.g., other days of the year or other days in January).
...
World knowledge can often be measured by having participants estimate quantities or give likelihood judgments in the same contexts without experimental manipulation [@FrankeEtAl2016].
...
Thus, we use our same RSA model to predict data for similar kinds of language tasks in a follow-up experiment; critically, the same model with the same prior knowledge is used to predict data from both experiments.
...
\mht{<-- this is the same sentence as the last sentence of the abstract}
<!--
...
\ndg{last paragraph should be clarified and simplified. otherwise intro is good.}
...
The vagueness and context-sensitivity of these adjectives can be modeled using a threshold semantics ($[[u]] = x > \theta$) \ndg{u not defined yet} where the threshold $\theta$ comes from an uninformative prior distribution and is inferred in context via pragmatic reasoning [@Lassiter2013; see also @Qing2014a].
<!---->
\begin{align}
...
\end{align}
<!---->
We use a speaker model that considers the alternatives of using the same utterance with the explicit comparison class (e.g., "It is warm relative to other days in Winter.", "It is warm relative to other days of the year."). 
...
\ndg{this section can be tightened up a bunch? also, don't need discussion to be separate section.}
...
\ndg{this isn't really a discussio. more a lead in and seguey to next section. so put in intro to next section? also need to simplify and clarify a bunch.}
...
\ndg{i'd rather leave some ambiguity: just say we use empirical frequency and that this is reasonable as a proxy for comparison cat usage frequency. in discussion can say that this could be a noisy channel completion thing or a conceptual priors thing.}
...
\ndg{there's some redundancy between this section and earlier stuff.}
...
\ndg{even stronger sell! modeling productive language yields productivity of experiment designs which pins down needed parameters....}
