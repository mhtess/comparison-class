...
In addition to the implicit comparison utterance, we assume the speaker has two alternative utterances corresponding to the usage of the adjective with an explicit comparison class $u_{sub}$ and $u_{super}$ (e.g., "It's warm relative to other days in winter." and "It's warm relative to other days of the year.").
The literal meanings of these alternatives are the same as the underspecified utterance (i.e., a threshold function: $$[\![u]\!] = x > \theta$$), but have the additional feature of overriding the value of the implicit comparison class $c_i$ that the pragmatic listener draws from her prior and forcing the literal listener into a particular comparison class via the function $\text{parseClass}$.
That is:
...
Consider the speaker model $S_1$ under different assumptions about the implicit comparison class: $S_{1}(u \mid x, \theta, c_i = c_{sub})$ and  $S_{1}(u \mid x, \theta, c_i = c_{super})$.
Figure\ \@(ref:speakerSimulations) shows the speaker production probabilities of the three possible utterances for each value along the degree scale (e.g., each temperature) and the two different possible implicit classes.
