---
title             : "Warm (for winter): Comparison class inference in vague language"
shorttitle        : "Comparison classes in vague language"
...
abstract: >
  ...
  *Warm* conveys that the temperature is high relative to some contextual *comparison class*, but little is known about how a listener decides upon the appropriate comparison class.
  Here, we formalize how world knowledge and pragmatic reasoning can drive the resolution of a comparison class.
  We introduce a Rational Speech Act model and derive two novel predictions from it.
  We validate these predictions using both a free production and a paraphrase experiment to measure listeners' beliefs about the likely comparison class used by a speaker.
...
A key challenge for listeners is that the comparison class often goes unsaid (e.g., in "It's warm outside.").

The existence of comparison classes for understanding vague language is uncontroversial [@Solt2009; @Bale2011].
Adult judgments of the felicity for adjectives like "dark" or "tall" depend upon fine-grained details of the statistics of the comparison class: When things in the comparison class are all very short, what counts as "tall" might not be very tall [@Schmidt2009; @Solt2012; @Qing2014].
...
Indeed, four-year-olds use their knowledge of kinds to differentiate between what goes into the comparison class and what does not:  Novel creatures (*pimwits*) are selected as either "tall" or "short" depending on the distribution of heights of *pimwits* and not the heights of other creatures like *daxes* [@Barner2008].
The problem is still underdetermiend, however, as any particular object of discourse, can be conceptualized or categorized in multiple ways, giving rise to multiple possible comparison classes (e.g., a pimwit is also a creature). 
...
We propose that listeners combine statistical, category knowledge with pragmatic reasoning to infer the comparison class implicitly used by the speaker.
...
Eq. \ref{eq:L1} - \ref{eq:L0} comprise a Rational Speech Act (RSA) model, a recursive Bayesian model where speaker $S$ and listener $L$ coordinate on an intended meaning of an utterance $u$ [for a review, see @Goodman2016].
...
In @Lassiter2013's model, $\theta$ comes from an uninform prior distribution over possible thresholds and is resolved by the listener reasoning about the different thresholds a speaker might be using $S_{1}(u \mid x, \theta)$ as well as the probabilities of different states of the world $P_{c}(x)$ (e.g., different temperatures).
Assuming the adjective adds some cost to the speaker's utterance (Eq. \ref{eq:S1}), the meaning of a gradale adjectives (e.g., "tall") is resolved by the pragmatic listener to mean something like "significantly taller than one might expect" [@Lassiter2015]. 
Critically, what "one might expect"---the prior distribution over world-states $P_{c}(x)$---is always relative to some comparison class $c$ (Eqs. \ref{eq:L1} \& \ref{eq:L0}).
Where does this comparison class come from?

We hypothesize that listeners maintain uncertainty about the comparison class and resolve likely comparison classes jointly while resolving the meaning of the utterance.
At a minimum, listeners use their world knowledge of what worlds are plausible under different comparison classes $P(x \mid c)$, what comparison classes are likely to be talked about *a priori* $P(c)$, and how a rational speaker would behave in a given world and comparison class $S_{1}(u \mid x, c, \theta)$ (Eq. \ref{eq:L1a}).
To explore this model, we consider an idealized case where the comparison class can be either a relatively specific (subordinate) or relatively general (superordinate) categorization (e.g., warm relative to *days in winter* or relative to *days of the year*).
...
We additionally assume the listener uses the more specific class to sample likely values of the degree (e.g., the listener's prior over temperatures is given by the distribution of temperatures for the specific class like *winter* $P(x \mid c = c_{specific})$). 
With these assumptions, the model becomes:

\begin{align}
L_{1}(x, c, \theta \mid u) &\propto S_{1}(u \mid x, c_{implicit}, \theta) \cdot P(x \mid c =  c_{specific}) \cdot P(c_{implicit}) \cdot P(\theta) \label{eq:L1a}\\
S_{1}(u \mid x, c_{implicit}, \theta) &\propto \exp{(\alpha_1 \cdot \ln {L_{0}(x \mid u, c_{literal}, \theta)}- \text{cost}(u)) } \label{eq:S1a}\\
L_{0}(x \mid u, c_{literal}, \theta) &\propto {\delta_{[\![u]\!](x, \theta)} \cdot P(x \mid c = c_{literal})} \label{eq:L0a}
\end{align}

We are interested in the behavior of the model with the underspecified utterance $u_{implicit}$ (e.g., "It's warm").
In addition to the underspecified utterance, we assume the speaker has two alternative utterances corresponding to the usage of the adjective with an explicit comparison class $u_{specific}$ and $u_{general}$ (e.g., "It's warm relative to other days in winter." and "It's warm relative to other days of the year.").
The literal meanings of these alternatives are the same as the underspecified utterance, but have the additionally feature of forcing the literal listener into a particular comparison class (potentially over-riding the value of $c$ that the pragmatic listener draws from her prior).
That is:

\begin{eqnarray}
c_{literal} & = &
\begin{cases}
c_{implicit} & \text{if } u = u_{implicit}\\
c_{explicit} & \text{if } u \in \{u_{specific}, u_{general}\} \\
\end{cases}
\end{eqnarray}

```{r fig.width=6, fig.cap='Speaker model behavior assuming different comparison classes. The underspecified utterance ("warm") takes on the same meaning of the explicit utterance whose comparison class is the same as the implicit class (e.g., if the speaker is using the (for winter) comparison class, "warm" means the same thing as "warm for winter"). When the speaker is assuming winter is the comparison class, when the temperture is low, she will say "warm" or "warm for winter", but if the temperature is relatively high, she will only say "warm for the year". When the speaker is assuming the year is the comparison class, and the temperature is relatively low, she will say "warm for winter". The listener believes temperature is relatively low (believes it is winter), and inverts this model of speaker to conclude that the implicit comparison class was winter. The specific class in this example (winter) is a Normal distribution centered at -1, and the general class (the year) is a Normal centered at 0.', fig.asp = 0.4}
...
To understand this model, we can consider two different speakers, which assume the two different comparison classes: $S_{1}(u \mid x, \theta, c = c_{specific})$ and  $S_{1}(u \mid x, \theta, c = c_{general})$.
(Note that even conditioned on a particular value for $c_{implicit}$, a speaker may over-ride this comparison class by producing one of the utterances with an explicit comparison class.) 
The underspecified utterance (implicit comparison class) takes on the same meaning as the explicit utterance $c_{implicit} = c_{explicit}$, where $c_{explicit}$ is derived from the utterance $u_{explicit}$.
...
A speaker's choice of what to say will depend upon the value of the degree $x$ and the threshold $\theta$ he is using, as well as the prior distributions over the degree $P(x \mid c)$ for each class $c_{specific}$ and $c_{general}$.
Consider the relative temperatures for the year ($c_{general}$) and for winter ($c_{specific}$) in a part of the world that exhibits seasons (e.g., Virginia, USA). 
Let's say in this case that $P(x \mid c_{general} = \mathcal{N}(0, 1)$ and $P(x \mid c_{specific} = \mathcal{N}(-1, 0.5)$,

For temperatures that are relatively low (e.g., temperatures in winter; roughly $x < -0.5$), the speaker will want to say the utterance with the specific comparison class (e.g, "warm for winter") or the implicit utterance (e.g., "warm") if the implicit class is the specific class.
