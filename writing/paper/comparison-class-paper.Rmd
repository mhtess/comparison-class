...
abstract: >
  Understanding language requires knowledge of the context.
  Yet, relevant context is often only implicit in everyday conversation.
...
Adult judgments of the felicity for adjectives like "dark" or "tall" depend upon fine-grained details of the statistics of the comparison class: When everyone is short, what counts as "tall" might be still not be very tall [@Schmidt2009; @Solt2012; @Qing2014].
How do you decide what goes into the comparison class?

A first guess at what the comparison class should be might be *other things of the same kind*.
Indeed, four-year-olds can use their knowledge of kinds to differentiate between what goes into the comparison class and what does not:  Novel creatures (*pimwits*) are selected as either "tall" or "short" depending on the distribution of heights of *pimwits* and not the heights of creatures that are not called *pimwits* [@Barner2008].
...
A day in January is also a day of the year; if it's warm, it could be *warm for the week*, *warm for winter*, or *warm for the year*.
In addition, comparison classes need not be conceptual: Two- to four-year-olds recognize that a mitten which is "big for a mitten" can still be "small" in the current perceptual context [@Ebeling1988].
Even more striking, a normatively "small hat" can be perceptually small yet still "big" for a doll (a la "too big"), a *functional* comparison [@Ebeling1994].

Here, we investigate the first aspect of this problem, deciding among multiple plausible, conceptual comparison classes.
...
The question of how listeners or speakers decide upon a comparison class has, to our knowledge, been addressed neither formally nor empirically.
In particular, we investigate how listeners decide upon a comparison class in undersupported contexts (e.g., just hearing "It's warm outside").


We propose that listeners can actively combine category knowledge with pragmatic reasoning to infer the comparison class implicitly used by the speaker.
We extend a Rational Speech Act (RSA) model for gradable adjectives [@Lassiter2013; @Lassiter2015] to allow it to flexibly reason about the implicit comparison class.
