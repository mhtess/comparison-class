---
title             : "Warm (for winter): Comparison classes in vague language"
...
Four-year-olds categorize novel creatures (*pimwits*) as either "tall" or "short" depending on the distribution of heights of *pimwits* and not the heights of creatures that are not called *pimwits*, suggesting the comparison class in that context is *other pimwits* [@Barner2008].
...
In these experiments, however, the comparison class is explicit: Participants are given exemplars of a distribution, and asked to derive a context-specific interpretation. 
...
How should a listener decide what comparison class a speaker is considering, when the comparison class is unsaid? 
To our knowledge, this question has been addressed neither formally nor empirically.^[
...
We propose that listeners actively combine category knowledge with pragmatic reasoning to infer the comparison class implicitly used by the speaker.

We extend a Rational Speech Act (RSA) model for gradable adjectives [@Lassiter2013] to allow it to flexibly reason about the implicit comparison class.
...
As with any Bayesian cognitive model, it is necessary to explicitly specify relevant prior knowledge (e.g., beliefs about temperatures) in order for the model to make **quantitative predictions**.
...
The vagueness and context-sensitivity of these adjectival utterances can be modeled using threshold semantics ($[\![u]\!] = x > \theta$, for utterance $u$, scalar degree $x$, and threshold $\theta$), where the threshold is probabilistically set with respect to a comparison class $c$
