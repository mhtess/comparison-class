...
In this framework, a pragmatic listener $L_1$ tries to resolve the state of the world $x$ (e.g., the temperature) from the utterance she heard $u$ (e.g., "it's warm").
...
In @Lassiter2013's model, $\theta$ comes from an uninformed prior distribution and is resolved by the listener reasoning about the different thresholds a speaker might be using $S_{1}(u \mid x, \theta)$ as well as the probabilities of different states of the world $P_{c}(x)$ (e.g., different temperatures).
It's been shown that by assuming that the adjective adds some cost to the speaker's utterance (Eq. \ref{eq:S1}), the meaning of positive-form adjectives (e.g., "tall") gets inferred to mean something like "significantly taller than one might expect *a priori*" [@Lassiter2015]. 
Critically, what "one might expect *a priori*" --- the prior distribution over world-states $P_{c}(x)$ --- is always relative to some comparison class $c$ (Eqs. \ref{eq:L1} \& \ref{eq:L0}) but where does the comparison class come from?

When a listener hears only that "it's warm outside" without an explicit comparison class (e.g., "...for the season"), we posit the listener infers the comparison class using her world knowledge of what worlds are plausible given different comparison classes $P(x \mid c)$ and what comparison classes are likely to be talked about $P(c)$, in addition to the already formalized process of how a rational speaker would behave in a given world and comparison class $S_{1}(u \mid x, c, \theta)$ (Eq. \ref{eq:L1a}).
...
We assume the listener is aware that the referent is a member of both classes and draws likely values of the degree (e.g., temperature) from the specific class prior $P(x \mid c = c_{specific})$. 
...
In addition to the underspecified utterance, we assume the speaker has two alternative utterances in which the comparison class is explicit $u_{specific}$ and $u_{general}$ (e.g., "It's warm relative to other days in winter." and "It's warm relative to other days of the year.").
These utterances have the effect of forcing the literal listener into a comparison class (potentially over-riding the value of $c$ that the pragmatic listener draws from her prior).
That is:
...
To understand this model, we can consider two different speakers, corresponding to the two different implicit comparison classes: $S_{1}(u \mid x, \theta, c = c_{specific})$ and  $S_{1}(u \mid x, \theta, c = c_{general})$.
Even conditioned on a particular value for $c_{implicit}$, a speaker can over-ride this comparison class by producing one of the utterances with an explicit comparison class. 
Thus, the utterance with the implicit comparison class will end up having the same meaning as the utterance with an explicit comparison class when $c_{implicit} = c_{explicit}$, where $c_{explicit}$ is derived from the utterance $u$.
