...
To understand the model behavior, consider the speaker model $S_1$ under different assumptions about the implicit comparison class: $S_{1}(u \mid x, \theta, c_i = c_{sub})$ and  $S_{1}(u \mid x, \theta, c_i = c_{super})$.
For illustrative purposes, the speaker model can produce sutterancs with adjectives of both the positive and negative forms (e.g., *warm* and *cold*).^[
  ...
  The pragmatic inferences concerning the comparison class we describe in this paper are invariant to whether or not the antonym is included in the alternative set. 
  The comparison class inference is also invariant to whether or not the antonym gets assigned its own unique threshold ($\theta_{cold}$), as well as whether or not the utterances with explicit comparison classes get assigned their own thresholds.
]
Figure\ \@(fig:speakerSimulations) shows the speaker production probabilities of the six possible utterances for each value along the degree scale (e.g., each temperature), and assuming the two different possible implicit classes.
The utterance with an implicit class takes on the same meaning as the utterance with the corresponding explicit class (i.e., if $c_{i} = c_{sub}$, then $u_i$ has the same informational content as $u_{sub}$).
Thus, reasoning about the likely comparison class reduces to reasoning about which explicit utterance the speaker would have been more likely to say.
For temperatures that are relatively low (e.g., temperatures in winter; roughly $x < -0.5$), the speaker will want to say the utterance with the subordinate class (e.g, "warm for winter") or the implicit utterance (e.g., "warm") if the implicit class is the subordinate class.
