...
\node[] at (4,2) {$\beta^0_0, \beta^1_0 \sim \text{Gaussian}(0, 2)$};
\node[] at (4,1.5) {$\beta^0_1, \beta^1_1 \sim \text{Uniform}(-3, 3)$};
\node[] at (4,1) {$z_{d, k} \sim \text{Bernoulli(0.5)}$};
\node[] at (4,0.5) {$\mu_{d, k}\sim \text{Uniform(-3, 3)}$};
\node[] at (4,0) {$\sigma_{d, k}\sim \text{Uniform(0, 3)}$};
\node[] at (4,-0.5) {$\alpha_1, \alpha_2 \sim \text{Uniform(0, 20)}$};

\node[] at (5,-1.5) {$\phi_{d,k} = \text{logistic} (\begin{cases} 
	\beta^0_0 +\beta^0_1\cdot \hat{f}_k  &\mbox{if } z_{k, d} = 0 \\ 
	\beta^1_0 +\beta^1_1\cdot \hat{f}_k  &\mbox{if } z_{k, d} = 1 
	\end{cases}$)};
\node[] at (4,-2.5) {$c_{d,k} \sim \text{Bernoulli}(\phi_{d,k})$};
\node[] at (4,-3) {$x_{d,k} \sim \text{Gaussian}(\mu_{d,k}, \sigma_{d,k})$};
