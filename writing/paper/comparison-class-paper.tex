...
All models do a good job at accommodating the Adjective Endorsement data.
...
We start with the global parameters. As a sanity check, the speaker optimality parameters for each task were inferred to be values for which listeners assuming a rational speaker and consistent with the prior literature on RSA models --- means and 95\% Bayesian credible intervals: $\alpha_1 =  \hdiresults{model_params_cis.csv}{intercept_slope_speakerOptimality_subordinateCC_NA}$, $\alpha_2 =  \hdiresults{model_params_cis.csv}{intercept_slope_speakerOptimality_adjEndorse_NA}$.
Recall that we inferred on a by-item basis whether each NP indicated a subordinate or basic level category, because we expect the basic-level bias to operate differently for these different regimes of items.

For NPs that were categorized as subordinate categories, the comparison class prior showed no basic-level bias -- $\beta^0_0 = \hdiresults{model_params_cis.csv}{intercept_slope_beta_intercept_sub_basic}$ -- and a positive effect of frequency of the noun phrase -- $\beta^0_1 = \hdiresults{model_params_cis.csv}{intercept_slope_beta_frequency_sub_basic}$.
 For NPs that were categorized as basic-level categories, the comparison class prior showed a strong basic-level bias -- $\beta^0_0 = \hdiresults{model_params_cis.csv}{intercept_slope_beta_intercept_basic_super}$ -- and a positive effect of frequency of the noun phrase -- $\beta^0_1 = \hdiresults{model_params_cis.csv}{intercept_slope_beta_frequency_basic_super}$.
