...
Whether or not something is \emph{warm} depends upon what \emph{reference class} you might be using (e.g., warm for Winter~vs.~warm for any season).  
The problem of determining an appropriate reference class touches on issues as wide as providing a price value to a house (CITE) to informing sentences for defendants convicted in the legal system (CITE).
The problem stems from how one would, in principle, calculate a frequency of a singular future event (i.e., to assign a probability to that event; CITE RACHEMBACH), but it extends to the assignment of values such in the legal cases.
The issue is that any particular event or thing belongs to multiple categories, giving rise to multiple possible reference classes. 

In natural language, the reference class problems takes shape when trying to assign a context-sensitive interpretation to statement with a relative meaning (e.g., \emph{warm}... relative to what?). 
This general problem of reference classes (in linguistics, called \emph{comparison classes}) touches on a wide swath of linguistic expressions, including relative adjectives \cite<\emph{warm} or \emph{tall};>{cresswell1976semantics, klein1980semantics, kennedy2005scale}, vague quantifiers \cite<e.g., \emph{many}, \emph{a lot};>{Scholler2017}, and, in a nonbovious way, category generalizations such as generic or habitual statements \cite<e.g., \emph{Robins lay eggs}, \emph{John smokes};>{Tessler2019psychrev}.
Formally, the reference class is a free (underspecified) variable that linguistic theories posit gets ``filled in by context'' \cite{kennedy2005scale, bale2008universal, Bale2011, Solt2009} \red{CHECK HOW THESE AUTHORS TALK ABOUT THAT}.
Psychological and/or communicative (pragmatic) mechanisms presumably are responsible for that filling in process, but how those mechanisms operate to determine a reference class remains obscure. 
...
Additionally, strong linguistic and perceptual cues can provide a signal to children as young as two-and-a-half that the comparison class can change \cite<e.g., an objectively small mitten can be \emph{big} relative to the tiny mittens on the table;>{Ebeling1988, Ebeling1994}.
A systematic investigation into how listeners flexibly adjust the reference class is outstanding. 


In what follows, we examine the problem of inferring references in communication using gradable adjectives like \emph{dark} or \emph{tall} as our case study. 
...
The model predicts an intuitive interaction between a listener's world knowledge (e.g., winter days are generally cold) and the valence/polarity of the adjective heard (e.g., \emph{warm}~vs.~\emph{cold}), which a control model without a Theory of Mind based reasoning mechanism does not predict.
The model also makes fine-grained qualitative predictions that can depend upon probabilistic knowledge about the property in the category (e.g., the temperature of Winter days) and prior beliefs about the reference class, a construct that has not been investigated to our knowledge.
We formalize further predictions about the structure of the reference class prior and show that it credibly contains elements that encode a basic level bias \cite{Rosch1975} and a notion of usage frequency.
For these investigations, we present empirical data from a large-scale, pre-registered, free-production experiment in which participants freely generate reference classes for interpreting relative adjectives in context.
...
When interpreting an adjective utterance $u$  (e.g., \emph{he's tall}) without an explicit comparison class $c$ (\emph{...relative to other C}), a listener is faced with the joint inference problem of determining the  degree $x$  being described by the adjective (e.g., the height of the referent) together with the comparison class $c$ assumed by the speaker when producing their utterance.
...
%
%
The listener in the mind of the speaker $L_{0}(x \mid u, c)$ knows the comparison class, and is simply a mapping (described below) from prior beliefs about the degree $x$ (e.g., height) to posterior beliefs about $x$ given a fully-specified adjectival utterance, an adjective and comparison class (e.g., \emph{tall for a basketball player}).
