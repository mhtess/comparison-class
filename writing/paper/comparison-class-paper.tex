...
\keywords{comparison class; reference class; pragmatics; Rational Speech Act; Bayesian cognitive model; Bayesian data analysis\newline\indent Word count: 6224}
\makeatletter
...
\abstract{
The meaning of an utterance can change depending on the context. Yet, what counts as context is often only implicit in everyday conversation. The utterance \emph{it’s warm outside} signals that the temperature outside is relatively high, but the temperature could be high relative to a number of different \emph{comparison classes}: other days of the year, other weeks, other seasons, etc. Theories of context-sensitivity in language agree that the comparison class is a crucial feature of meaning understanding, but little is known about how a listener decides upon a comparison class.
...
Our model presents a first model-driven step to resolving the implicit aspects of ordinary language use. 
...
Whether or not something is \emph{warm} depends upon what \emph{comparison class} a speaker might be using (e.g., warm for Winter~vs.~warm for any season), though speakers rarely articulate comparison classes explicitly.
The problem of determining the comparison class is a special case of the \emph{reference class problem}, originally developed in the domain of probability and statistics when one wants to calculate a frequency for a singular future event \cite<i.e., to assign a probability to that event;>{Reichenbach1949, hajek2007reference}.
...
The common issue is that any particular entity or event belongs to multiple categories, giving rise to multiple possible reference classes, which could imply different probabilities, price values, or even time spent in jail.

In natural language, the problem of reference or comparison classes is front and center when interpreting \emph{relative statements} (e.g., warm relative to what?). 
...
Formally, the comparison class is a free (underspecified) variable that linguistic theories assume gets filled in by context \cite{kennedy2005scale, bale2008universal, Bale2011, Solt2009}.
...
Comparison classes thus also serve as a case study into the representations that sit at the interface of psychological and linguistic theory.
...
We describe how comparison classes can be incorporated into computational models of pragmatic communication, using a Theory of Mind based reasoning mechanism.
...
The model also makes fine-grained quantitative predictions that depend upon distributional knowledge about the property in the category (e.g., the temperature of Winter days) as well as prior beliefs about the comparison class.
...
Thus, for our primary analysis, we automatically categorize responses as ``subordinate'' if the preprocessed response contains the subordinate-NP as a substring (\rlgetnum{mention_category_counts.csv}{type}{Subordinate_notSuperordinate}{n}{1}, or \rlgetnum{mention_category_counts.csv}{type}{Subordinate_notSuperordinate}{prop}{1}\% of the response); all other responses are coded as ``superordinate'' (10596, or 40.3\%).
...
Since we infer the world knowledge priors $P(x)$, our approach might appear like a \emph{descriptive Bayesian approach} \cite{tauber2017}, in which the modeler is simply interested in determining whether the inference patterns  observed in the data can be explained by prior knowledge (in the way articulated by the model) and determining what that prior knowledge would have to be to capture the data.
Instead, we aim to predict (not merely accommodate) the item-level variation by way of our model and participants' world knowledge about categories and properties.
...
For this reason, we take a different approach: We take advantage of the fact that our computational model understands a productive fragment of natural language, and so the model can be used to predict data from an experiment about a similar set of natural language expressions ($n=375$, \emph{Adjective Endorsements}: truth judgments for sentences with explicit comparison classes involving the same scalar adjectives applied to the same categories, e.g., ``Imagine a day in winter. Do you think it is cold relative to other days of the year?''; see SI for details). We then synthesize the two data sets using a single Bayesian data analytic model, wherein the parameters that govern prior knowledge in the pragmatics models are constrained by both data sets.
...
\begin{figure}[t!]
    \centering
      \subfigure[Human data and model predictions for comparison class inferences.]{\label{fig:residuals}\includegraphics[width=\textwidth]{figs/bars_ccinfOnly_finalExpt_byNP_exampleItems_intercept_slope_300k.pdf}} \\
    \subfigure[Imputed prior distributions over degrees.]{\label{fig:worldPriors}\includegraphics[width=\textwidth]{figs/reconstructed_world_priors2.pdf}} 
...
For example, the model predicts variability in the inferences from the weak/strong \emph{teenagers} and \emph{adults}, where no such variability is found (also, for the entire category of \emph{spices}, see Fig.~S7). 
The variability in inferences about the comparison classes for the strength of different walls is largely a result of a bias for \emph{tents} being a very good comparison class whereas \emph{condos} and \emph{mansions} are strongly preferred (Fig.~S7).
...
We implement this proposal by extending a state-of-the-art language understanding model. Its prediction are empirically confirmed in an open-ended response measure using a diverse range of linguistic stimuli.
...
We have addressed a particular aspect of comparison class inference: deciding among two conceptually-based comparison classes, that we pre-specified as subordinate~vs.~superordinate category labels.
The restriction of our model to considering only two comparison classes was a convenient simplification, but the fact that the majority of responses naturally produced were either subordinate~vs.~superordinate category labels suggests that this was also a reasonable simplification. 
...
The comparison class prior in our model could be elaborated to encode a more structured theory about how the hypothesis space of comparison classes is constructed (e.g., via a rational clustering type model \emph{a la} \citeNP{kemp2012integrated}).
...
Our modeling work pointed to a basic-level bias and corpus frequency as two factors that inform the hypothesis space of comparison class, but developing a more general theory of the comparison class prior will be an important step for this approach.
Many of the ``miscellaneous'' comparison classes that were elicited in our task appealed to other aspects of the context that were made available by the language in the task.
...
This fully Bayesian language approach also provides further constraints on the language understanding models, which must predict quantitative data from two related language experiments.
