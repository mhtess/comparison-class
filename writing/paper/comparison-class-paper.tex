...
\ndg{whatsa hybrid blah blah?} We introduce a hybrid experimental-Bayesian data analytic approach to examine the quantitative predictions of the model, finding that usage frequency and a preference for basic-level categories are two main contributors to comparison class inference.
...
With a Bayesian formulation, a listener combines these prior expectations with the likelihood that a speaker would use an adjective to describe a member of a particular category.\footnote{We use the male pronoun to refer to the listener and the female pronoun to refer to the speaker, for clarity of exposition. \ndg{funny place for this footnote -- no pronouns used yet?}}
...
We processed the responses by correcting for misspellings  (569; 2.14\%), lemmatizing (867, 3.28\%), and collapsing across synonyms (323, 1.22\%; see SI for details). 
...
Our primary hypothesis was that usage of a subordinate~vs.~superordinate comparison class would be influenced via an interaction between the general expectations about the category and the polarity of the adjective (Figure \ref{fig:ccInferenceItems} \ndg{did you mean to point to the model fig above?}), which we tested with a maximal Bayesian mixed-effects model.\footnote{
...
The opposite interaction was observed for categories that were expected to be near the low-end of the scale (e.g., gymnast player \ndg{?}): Hearing the positive-form adjective led to credibly more subordinate inferences than hearing a negative-form adjective, in comparison to the middle-of-the-scale subordinate categories:  \brmresults{expt3_brm_pilot.csv}{np_expectationslow:adj_polarity1}.
...
\ndg{i can imagine reviewers complaining that this is circular, since the truth judgements depend on the same prior knowledge and in a similar way -- so at most our model is simply jointly accomodating the data.... what do you think?}
...
The fact that participants naturally produce different responses to the comparison class elicitation task suggests that these inferences are easily accessible and can arise spontaneously. \ndg{why?}
...
In our maximal model, we see an overall weak preference for basic-level categories over subordinate level categories and so the model does not show a strong preference for \emph{spices}. 
...
\ndg{cite piantadosi, tily, gibson here for the functional reasons that language embraces such ambiguity?}
