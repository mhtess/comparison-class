...
As a proxy for comparison class usage frequency, we use empirical (log) frequency of the noun phrase \(\hat{f}_k\)  estimated from the Google WebGram corpus and the length of the noun phrase in characters $|k|$, each scaled by a free parameter $\beta_{freq}$,  $\beta_{length}$ such that $P(c) \propto \exp{(\beta_{freq} \cdot \log \hat{f}_k - \beta_{length} \cdot |k|)}$.
