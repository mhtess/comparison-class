...
\shorttitle{ Inferring reference classes}
\affiliation{
\vspace{0.5cm}
...
\keywords{reference class; comparison class; pragmatics; Rational Speech Act; Bayesian cognitive model; Bayesian data analysis\newline\indent Word count: 6224}
\makeatletter
...
\authornote{Draft version 9/15/2020. This paper has not been peer reviewed. %Please do not copy or cite without author's permission. 
...
We explore this problem in natural language, where the reference class problem manifests in a wide variety of context-sensitive linguistic expressions. 
...
Using the case study of gradable adjectives (e.g., \emph{warm}), we extend a Bayesian model of pragmatic reasoning to reason flexibly about the reference class and test its qualitative predictions in a large-scale free-production experiment.
We compare models that infer the reference class with and without a Theory of Mind based reasoning mechanisms, finding that the Theory of Mind mechanism is necessary to derive the correct qualitative predictions. 
 We then quantitatively synthesize the model and data using Bayesian data analysis, which further reveals  that usage frequency and a preference for basic-level categories are two main contributors to reference class inference.
Our model presents a first sketch of a solution to resolving the implicit aspects of ordinary language use. 
...
Whether or not something is \emph{warm} depends upon what \emph{reference class} you might be using (e.g., warm for Winter~vs.~warm for any season), though references classes are almost never articulated explicitly.
The problem of determining an appropriate reference class touches on broad societal issues ranging from providing a price valuation for a house (e.g., by appealing to the values of ``similar'' houses) to informing the severity of sentences for defendants convicted in the legal system \cite<e.g., by appealing to ``similar'' convictions;>{colyvan2001crime, cheng2009practical}.
In the domain of probability and statistics, the problem crops up when one wants to calculate a frequency for a singular future event \cite<i.e., to assign a probability to that event;>{Reichenbach1949, hajek2007reference}.
The basic issue is that any particular event or thing belongs to multiple categories, giving rise to multiple possible reference classes, which could imply different price values, probabilities, or even time spent in jail.

In natural language, the problem of references class  (in linguistics, also called \emph{comparison classes}) is front and center when interpreting \emph{relative statements} (e.g., warm relative to what?). 
The problem thus touches on a wide swath of linguistic expressions, including adjectives \cite<\emph{warm}, \emph{tall};>{bartsch1974semantic, cresswell1976semantics, klein1980semantics, kennedy2005scale}, quantifiers \cite<e.g., \emph{many}, \emph{a lot};>{Scholler2017}, and, in a nonobvious way, category generalizations such as generic or habitual statements \cite<e.g., \emph{Robins lay eggs}, \emph{John smokes};>{Tessler2019psychrev}.
Formally, the reference class is a free (underspecified) variable that linguistic theories assume gets filled in by context \cite{kennedy2005scale, bale2008universal, Bale2011, Solt2009}.
Psychological and/or communicative mechanisms presumably are responsible for that filling in process, but how those mechanisms operate to determine a reference class remains obscure. 
References classes thus also serve as a case study into the representations that sit at the interface of psychological and linguistic theory.

Empirically, the question of how human listeners decide upon the reference class has also received little systematic attention.
Empirical work with adults and children has primarily interrogated how judgments and interpretations for relative adjectives (e.g., \emph{dark}, \emph{tall}) depend upon the statistical details of a pre-determined reference class  \cite{Barner2008, Qing2014, Schmidt2009, Solt2012}.
Indirect evidence that the reference class is flexibly inferred comes from studies with young children, which find that a directly-modified noun phrase can be used to constrain what objects go into the reference class: What counts as a \emph{tall pimwit} (a novel category) depends on the distribution of heights of \emph{pimwits} and not the heights of other categories like \emph{daxes} \cite<i.e., \emph{daxes} are not in the reference class;>{Barner2008}.
Additionally, strong linguistic and perceptual cues can provide a signal to children as young as two-and-a-half that the reference class can change \cite<e.g., an objectively small mitten can be \emph{big}, if there are many tiny mittens on the table;>{Ebeling1988, Ebeling1994}.
Still, a systematic investigation into how listeners flexibly adjust the reference class is outstanding. 


In what follows, we examine the problem of inferring reference classes in communication using gradable adjectives like \emph{warm} or \emph{tall} as our case study. 
We describe how reference classes can be incorporated into computational models of pragmatic communication, using a Theory of Mind based reasoning mechanism.
The model predicts an intuitive interaction between a listener's world knowledge (e.g., winter days are generally cold) and the valence (or, polarity) of the adjective heard (e.g., \emph{warm}~vs.~\emph{cold}), which a control model without a Theory of Mind based reasoning mechanism does not predict.
The model also makes fine-grained quantitative predictions that depend upon distributional knowledge about the property in the category (e.g., the temperature of Winter days) as well as prior beliefs about the reference class.
We formalize further predictions about the structure of the reference class prior and show that it credibly contains elements that encode a basic level bias \cite{rosch1975family} and a notion of usage frequency.
For these investigations, we present empirical data from a pre-registered, large-scale free-production experiment in which participants freely generate reference classes for interpreting relative adjectives in context.
...
Here we articulate the computations that underly reference class inference in communication and how that inference interfaces with standard linguistic theories of adjective meaning.
...
Eq. \ref{eq:L1a} describes a pragmatic listener $L_1$ updating his beliefs about the degree $x$ and the reference class $c$ by assuming that the speaker $S_1$ intentionally produced an adjectival utterance $u$ so that she could communicate about the degree (e.g., height).
%The lis in this case is assuming , assuming the comparison class,
The prior distribution of reference classes $P(c \mid k)$ is assumed to be in common ground, constrained by prior knowledge about categories and the situation (e.g.,~that the referent is a basketball player; a basketball player is a person; ...).
Though reference classes can be constructed in various ways, including sets of objects in the perceptual environment (e.g.,~\emph{big relative to the things around it}) or from the hypothetical functions of an object \cite<e.g., \emph{this shirt is big for the doll;}>{Ebeling1994}, we restrict our analysis to a hypothesis space of reference classes constructed out of a taxonomic hierarchy (i.e., \emph{conceptual reference classes}), with the lowest level of hierarchy being the subordinate category to which the referent belongs (e.g., a basketball player; Figure \ref{fig:modelCartoon}A). 
...
%
%
The listener in the mind of the speaker $L_{0}(x \mid u, c)$ is assumed to know the reference class and is simply a mapping (described below) from prior beliefs about the degree $x$ (e.g., height) to posterior beliefs about $x$ given a fully-specified adjectival utterance (i.e., an adjective and reference class e.g., \emph{tall for a basketball player}).
...
We use a model of a literal listener which, following standard treatment in formal semantics \cite<e.g.,>{Kennedy2007}, takes the literal meaning of a gradable adjective to be simply a threshold function on the degree (e.g., \([\![tall]\!] = {x>\theta}\)) which is combined with the listener's prior knowledge of the degree in the reference class $P(x \mid c)$ to produce an interpretation of the adjective specific to that reference class (Figure \ref{fig:modelCartoon}B).\footnote{Following standard treatment of antonyms, the
  ...
  inferences about the reference class that are the focus of this paper
  ...
  whether or not the antonym is assigned its own unique threshold
  (\(\theta_{short}\)).}
  %
 \begin{align}
...
\end{align}
%
Equation \ref{eq:L0} is a model of literal listener who updates their prior beliefs about the degree given a reference class $P(x\mid c)$ via a threshold function, represented by the Kronecker delta function \(\delta_{\mbox{ $[\![ u ]\!]$}(x, \theta)}\) that returns \(1\) when the utterance is true (i.e., when \(x > \theta\)) and \(0\) otherwise.
...
\caption{\small \label{fig:modelCartoon}Model overview. A listener hears a basketball player described as tall by a speaker. A: A hypothesis space of reference classes is constructed over a taxonomic hierarchy. B: A reference class (or, comparison class) is realized as a probability distribution over the relevant degree (e.g., height; shown in black). The interpretation of a gradable adjective (e.g., \emph{tall}; shown in red)---given by $L_{0}(x \mid u, c)$---is specific to the reference class (facets). C: Listener $L_1$ imagines what a speaker $S_1$ would say given different heights of the referent (x-axis) and assuming different reference classes (facets); the opacity of the lines is proportional to the listener's prior beliefs of that particular height (e.g., the listener knows the referent is a basketball player; thus, heights towards the upper range of the scale are \emph{a priori} more likely). D: Averaging over the plausible heights of the referent, the speaker would only have a preference to say \emph{tall} over \emph{short} if the reference class were \emph{people}. E: The pragmatic listener infers that \emph{people} is more likely to be the reference class given that the speaker said \emph{tall}. If the referent is described as \emph{short}, the listener infers the speaker meant \emph{short for a basketball player}. Schematic prior distribution of heights for people is a unit normal distribution $\mathcal{N}(0, 1)$ and the heights of basketball players is a right-shifted normal with smaller variance $\mathcal{N}(0.5, 0.5)$.
...
We consider an idealized case where there are only two reference classes: a relatively subordinate-level category ($c_{sub}$) or a relatively superordinate-level category ($c_{super}$) (e.g., tall relative to \emph{basketball players} or relative to \emph{people}): \(c \in \{c_{sub}, c_{super}\}\).
Figure \ref{fig:modelCartoon}C shows the speaker production probabilities of the three possible utterances (\emph{tall}, \emph{short}, silence) for each value along the degree scale (e.g., each height) for each of the two different reference classes (facets).
If the reference class is \emph{basketball players}, the speaker will be reluctant to produce \emph{tall} until the height is substantially greater than average basketball player (red line shifted to the right with respect to the \emph{person} comparison class).
The listener uses Bayes' rule to ``invert'' this generative model of the utterance (i.e., the speaker model), inferring the implicit reference class.
...
Averaging over the plausible heights of a basketball player, the listener reasons that a speaker who says \emph{tall} would be more likely to do so assuming the reference class of \emph{people}  (Figure \ref{fig:modelCartoon}D).
...
In the above model, the reference class variable $c$ takes shape as an another interlocutor's (the speaker's) beliefs. 
One might wonder whether the reference class variable needs to appeal to another's beliefs, or if it instead be could be a variable just within the mind of the listener. 
We can address this question by reformulating the reference class inference spelled out in Eq.~\ref{eq:L1a} in terms of a literal listener model (\emph{a la} Eq.~\ref{eq:L0}). 
...
\end{align}
%
Similar to the pragmatic listener model (Eq.~\ref{eq:L1a}), this listener can use their knowledge of the referent $k$ to constrain the hypothesis space of reference classes via $P(c \mid k)$ (e.g., with the knowledge that the referent is a basketball player, consider reference classes that are the same as or superordinate to the class of basketball players).
Unlike the pragmatic listener model (Eq.~\ref{eq:L1a}), however, the literal listener version of the model does not hold different representations of the referent in mind: The pragmatic listener has their private representation of the referent---given by the prior distribution of the degree $P(x \mid k)$---and imagines a speaker who acts assuming some reference class---$S_1(u \mid c)$---where $c$ and $k$ may or may not index the same class (e.g., the listener may know the referent is a basketball player -- $k = \emph{basketball players}$ -- but believes the speaker was assuming a \emph{person} comparison class -- $c = \emph{people}$).
The literal listener version of the model has no way of separating these representations.
In effect, this listener is answering a different question from the reference class inference problem. The question this alternative model is answering is: what is more likely---a basketball player whose height is very great or a person whose height is very great? 
...
Thus, reference classes in communication require a Theory of Mind based reasoning mechanism in order to resolve in context. 
...
We test the model prediction that the reference class can be flexibly adjusted based on world knowledge and pragmatic principles in a large-scale, web-based free-production experiment.
...
To norm world knowledge needed in our quantitative modeling, we also measure truth judgments for statements involving scalar adjectives and an explicit reference class (see SI).
%We model both data sets using the same RSA architecture.
Sample size, exclusion criteria, regression analysis, and cognitive model analysis were preregistered for the Reference Class Inference task: \url{osf.io/xuc96}.
...
This number was arrived at with the goal of estimating the probability of a subordinate~vs.~superordinate reference class with confidence intervals no larger than 0.20 for each unique item in the data set, which conservatively produced an estimate of roughly 50 responses per item.
...
We preprocessed the remaining 26,557 responses by correcting for misspellings  (462; 1.7\%), lemmatizing (3770, 14.2\%), and collapsing across synonyms (660, 2.5\%; see SI for details).
...
Pilot testing suggested that participants primarily provide comparison class paraphrases that are identical to the subordinate noun phrase by which the referent is introduced (``subordinate-NP'', e.g., a basketball player) or a more superordinate category (``superordinate-NP'').
Thus, for our primary analysis, we automatically categorize responses as ``subordinate'' if the preprocessed response contains the subordinate-NP as a substring (15721, or 59.7\% of the response); all other responses are coded as ``superordinate'' (10596, or 40.3\%).
This analysis assumes that in order to convey a category at least as specific as the subordinate-NP, one must include the subordinate-NP in the response (e.g., \emph{male basketball players} is more specific than \emph{basketball players} and includes the substring \emph{basketball player}).\footnote{Negation markers (e.g., ``all kinds of people except basketball players'') are one case where this assumption would be invalid, but no responses used negation markers in this way.}
...
To examine the free responses in more detail, we performed a secondary analysis where we categorized the superordinate responses as to whether or not they contained the modal superordinate response for that item.\footnote{We determined the modal superordinate-NP by generating what we experimenters believed was the most salient superordinate category. Out of our 90 pre-conceived superordinate-NPs, one (\emph{desserts}) turned out to be an incorrect guess as to what would be the most salient superordinate category for participants (participants instead responsed \emph{food} when asked about \emph{smoothies}, \emph{muffins}, \emph{chocolate fondue}). We replaced this pre-designated superord inate-NP with the empirical modal superordinate-NP (\emph{foods}) for purposes of this analysis.}
Of the 10596 responses categorized as ``superordinate'', 6645 (25.2\% of all responses) explicitly mention the modal superordinate-NP for that item and 3951 (15.0\%) mention neither the subordinate nor the modal superordinate-NP (``other'' responses).
...
As predicted by the comparison class inference model, we observe a pair of two-way interactions (Figure \ref{fig:ccInferenceMain}):
...
This interaction was the result of the high-end-of-the-scale subordinate categories showing more subordinate comparison class inferences for the negative adjective (e.g., short) than for the positive adjective (e.g., tall): \brmresults{expt_brm_contrasts_10k_2.csv}{highneg_highpos}.
The same preference was not observed for the middle-of-the-scale, control items (e.g., \emph{soccer players}; \brmresults{expt_brm_contrasts_10k_2.csv}{adjpol}). 
...
Additional variance in inferences, which cannot be explained by world knowledge, can be captured in the model via the comparison class prior $P(c \mid k$), which encodes baseline expectations about the comparison class given the category membership $k$ of the referent.
...
Contra a fully descriptive approach, we are not interested in whether the variability can be explained by some knowledge in principle, but whether the variability is explained by actual knowledge about the categories and properties.
...
For this reason, we take a different approach: We utilize the productivity of the fragment of natural language that our computational model understands in order to predict data from a related language experiment ($n=375$, \emph{Adjective Endorsements}: truth judgments for sentences with explicit comparison classes involving the same scalar adjectives applied to the same categories, e.g., ``Imagine a day in winter. Do you think it is cold relative to other days of the year?''; see SI for details and see Figure \ref{fig:residuals} for results for eight items) and synthesize the two data sets using a single Bayesian data analytic model, wherein the parameters that govern background knowledge in the pragmatics models are constrained by both data sets.
...
We can use these parameters to compute imputed prior probabilities of subordinate, basic, and superordinate comparison classes (assuming a constant effect of usage frequency).
...
The model is thus be similar in structure to models of presupposition accommodation \cite<e.g.,>{Qing2016projective}, which linguistic theories classically pose as involving a listener adding or revising information to the common ground in order to make sense of an utterance. For example, if John says to Mary ``My car is in the shop'', it is not necessary \emph{a priori} that Mary know that John has a car; if Mary did not know (or she temporarily forgot) that John owns a car, Mary can \emph{accommodate} John's utterance by adding to the common ground the presupposition that \emph{John has a car}.
...
A speaker would only utter such a sentence if they believed either (i) the comparison class was already in the common ground or (ii) the listener could reasonably be assumed to infer the comparison class.
...
In the SI, we show how the inference about the comparison class occurs regardless of whether the listener assumes the speaker was presupposing the comparison class to already be in the common ground or if they assumed the listener could reconstruct the intended comparison class.
...
This fully Bayesian language approach also provides further constraints on the language understanding models, which must predict quantitative data from two related language experiments, only one of which requires comparison class inference.
