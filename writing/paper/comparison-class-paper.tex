...
To explicate our model, imagine hearing a basketball player described as either \emph{tall} or \emph{short} (Figure \ref{fig:modelCartoon}). 
...
The hypothesis space of comparison classes is assumed to be in common ground, constrained by prior knowledge and common knowledge about the situation (e.g.~that the referent is a basketball player).
%beliefs about the referent that are shared between the speaker and listener: A speaker  cannot be assumed to intend a comparison class about which she has no knowledge.
...
In contrast to the space of comparison classes, the hypothesis space of degrees (e.g., heights) is given by the semantics of the adjectives (e.g., \emph{tall} $\rightarrow$ height). % and is informed by the totality of the listener's relevant beliefs. 
...
\end{align}
%
\ndg{given that we don't use this formulation, how about moving it (and the discussion of shared vs private information) into a footnote?}
...
The listener uses Bayes' rule to `invert' this generative model of the utterance (i.e., the speaker model), inferring the implicit comparison class.
 They combine the speaker model with their prior knowledge about the height of the referent: The listener knows the referent is a basketball player and thus their height is distributed according to the basketball player distribution (Figure \ref{fig:modelCartoon}B, bottom, black; this distribution is superimposed as an opacity on Figure \ref{fig:modelCartoon}C).
...
In this large set of items, we expected many judgements to have small variance, requiring relatively fewer data points to estimate reliably.
...
For those items that received exceedingly consistent responses we stopped data collection at 35 responses. This allowed us to focus resources on providing better estimates for the items with more variability in responses. 
...
As predicted, \ndg{incomprehensible:} the usage of a specific~vs.~general comparison class (i.e., paraphrases that mentioned the specific NP~vs.~not)  was influenced via an interaction between the general expectations about the specific category and the adjective polarity (Figure \ref{fig:ccInferenceItems}).\footnote{
