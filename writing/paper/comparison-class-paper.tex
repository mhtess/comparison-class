...
\subsection{Experiment 2: Comparison class inference}
...
The experimental materials were a modified subset of those generated by participants in Experiment 1. 
In total, we used 90 sets of stimuli: each set contains 3 specific categories, which varied in their general expectations (low, medium, high) within the same general category (e.g., basketball player [high], soccer player [medium], jockey [low]; general category: people). 
The categories within each set of stimuli could be paired with either a positive-form or negative-form adjective (e.g., tall or short). 
Thus, this experiment had 90 sets $\times$ 3 levels of general expectations (specific categories) $\times$ 2 adjectives = 540 unique items. 

We used a \emph{context sentence} to introduce the referent as a member of the specific category and provide an appropriate but minimal context in which the adjective could be uttered (e.g., \emph{John sees a \{basketball player, golfer, jockey\}}); these context sentences were the same for all 3 categories in an item set.

The adjectival utterance predicated the adjective of a pronoun used to refer to the referent (e.g., \emph{John says: ``They're tall''}). We had the speaker use a pronoun so as to provide a strong linguistic cue as to the intended comparison class. If the referent was a person, we used to the singular ``they'' to refer to them; otherwise, we used either ``it'' or ``they'' depending on the plurality of the referent. 
...
\begin{quote}
\emph{They're tall relative to other \_\_\_\_\_\_.}
\end{quote}
...
We used a sequential sampling method for items, wherein we paused data collection after collecting 35 responses on each item.
We then analyzed the partial data set on a by-item basis to see which (if any) items had received at least exceedingly consistent responses, which we defined to be 33 out of 35  ($>94\%$) of the same response. 
For those items that received exceedingly consistent responses, we stopped data collected at 35 responses in order to focus resources to get a better estimate of those items with more variability in responses. 
...
The screen had a sentence in which a named speaker (e.g., Joseph) says to a named listener (e.g., Elizabeth) ``It's a beautiful day, isn't it?''. Participants were asked to type in a text box to whom the speaker (Joseph) is talking (i.e., Elizabeth).
Names used were randomized and could not be read off the source .html file.
...
After the language comprehension test, participants read instructions about the task. They were told they they would be asked to rephrase something a person said, that the person said a word that is relative, and that their task was to figure out what the word was relative to. They were given the example of \emph{John says: ``The Empire State Building is tall''} and asked to fill-in the same kind of response they would do on the main trials (i.e., \emph{The Empire State Building is tall relative to other \_\_\_}). Participants were told to fill in the blank with a group or category that makes the most sense and use their common sense.
...
Participants then completed 36 main trials, described in the materials section.
...
\subsection{Experiment 3: Adjective endorsement}
