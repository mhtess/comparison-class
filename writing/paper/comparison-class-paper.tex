...
We then quantitatively synthesize the model with the data using Bayesian data analysis, which further reveals  that usage frequency and a preference for basic-level categories are two main contributors to comparison class inference.
...
Indirect evidence that the comparison class can be flexibly adjusted comes from studies with young children, which find that a directly-modified noun phrase can be used to constrain what objects goes go into the comparison class: What counts as a \emph{tall pimwit} (a novel category) depends on the distribution of heights of \emph{pimwits} and not the heights of other categories like \emph{daxes} \cite{Barner2008}.
Additionally, strong linguistic and perceptual cues can provide a signal to children as young as 2-and-a-half that the comparison class can change \cite<e.g., an objectively small mitten can be \emph{big} relative to the tiny mittens on the table;>{Ebeling1988, Ebeling1994}.
...
We test this hypothesis in a large-scale, pre-registered, free-production experiment in which participants are free to describe a comparison class of their choosing.
...
The hypothesis space \ndg{prior?} of comparison classes $P(c \mid k)$ is assumed to be in common ground, constrained by prior knowledge and common knowledge about the situation (e.g.,~that the referent is a basketball player).
...
%
For simplicity, we assume the speaker has three utterances she can say: \{\emph{tall}, \emph{short}, silence\}, where silence is a semantically vacuous utterance (i.e., a null action).\footnote{We use the male pronoun to refer to the listener and the female pronoun to refer to the speaker, for clarity of exposition.}\footnote{The inferences we model are invariant to reasonable choices of alternative utterances. Most notably, including alternatives that allow the speaker to explicitly communicate the comparison class (e.g., \emph{tall for a basketball player}) do not change the qualitative results we describe.
}
...
When the subordinate category was expected to be near the high-end of the scale (e.g., basketball player), the positive-form adjective (e.g., tall) led to more subordinate comparison classes than the negative-form adjective (e.g., short) in comparison to the control, middle-of-the-scale items (e.g., soccer player): posterior mean beta-weight and 95\% Bayesian credible interval: \brmresults{expt_brm_contrasts_10k.csv}{adjpol_nphigh}.
This interaction was the result of the high-end-of-the-scale subordinate categories showing more subordinate comparison class inferences for the negative adjective (e.g., short) than for the positive adjective (e.g., tall): \brmresults{expt_brm_contrasts_10k.csv}{highneg_highpos}.
The same preference was not observed for the middle-of-the-scale, control items (e.g., \emph{soccer players}; \brmresults{expt_brm_contrasts_10k.csv}{adjpol}). 
A comparable interaction was observed for categories that were expected to be near the low-end of the scale (e.g., jockey): Hearing the positive-form adjective led to credibly more subordinate inferences than hearing a negative-form adjective, in comparison to the middle-of-the-scale subordinate categories:  \brmresults{expt_brm_contrasts_10k.csv}{adjpol_nplow}.
Again, this was driven by the behavior of the low-end-of-the-scale subordinate categories, which showed a stronger preference for the superordinate comparison classes for negative than for positive adjectives (\brmresults{expt_brm_contrasts_10k.csv}{lowneg_lowpos}).

Our results are highly symmetric across the low-end~vs.~high-end of the scale subordinate categories.
We observe an overall preference for subordinate comparison classes for the control (middle-of-the-scale) subordinate categories (e.g., \emph{soccer players}; \brmresults{expt_brm_contrasts_10k.csv}{intercept}) and no overall differences in this preference for items at the high-end of the scale (e.g., \emph{basketball players}; \brmresults{expt_brm_contrasts_10k.csv}{nphigh}) or the low-end of the scale (e.g., \emph{gymnasts}; \brmresults{expt_brm_contrasts_10k.csv}{nplow}). 
...
The inferences that result from adjectives that are in conflict with a listener's general expectations of the categories (e.g., tall gymnasts~vs.~short basketball players) were not different between the low-end and high-end items (\brmresults{expt_brm_contrasts_10k.csv}{lowpos_highneg}), nor were the inferences  from adjectives that were consistent with general expectations about a category (e.g., short gymnasts~vs.~tall basketball players; \brmresults{expt_brm_contrasts_10k.csv}{lowneg_highpos}).
...
Additional variance in inferences, which cannot be explained by world knowledge, can be captured in the model via the comparison class prior $P(c \mid k$), which encodes baseline expectations about the comparison class given the category membership of the referent.
...
This method is not useful for our purposes since we are interested in characterizing the extent of the variability in comparison class inferences that are explained by actual knowledge about the categories and properties, not whether the variability can be be explained by some knowledge in principle. (That is, we would like to \emph{predict} not merely \emph{accomodate} item-level variation.)
...
For this reason, we take a different approach: We utilize the productivity of the fragment of natural language that our computational model understands to predict data from a related language experiment (\emph{Adjective Endorsements}: truth judgments for statements with explicit comparison classes involving the same scalar adjectives applied to the same categories; $n=375$, see SI for details and see Figure \ref{fig:residuals} for results for eight items) and synthesize the two data sets using a single Bayesian data analytic model, wherein the parameters that govern background knowledge in the pragmatics models are constrained by both data sets.
...
\begin{center}
  \begin{table}[h]
    ...
    columns/subordinateCC_r2/.style={string type, column name={$r^2$ CC}, column type = l,  sci sep align, precision=3},
   columns/subordinateCC_mse/.style={fixed, column name={$MSE$ CC}, column type = l, dec sep align, precision=4},
    columns/adjEndorse_r2/.style={string type, column name={$r^2$ norming}, column type = l, sci sep align, precision=3},
     columns/adjEndorse_mse/.style={fixed, column name={$MSE$ norming}, column type = l, dec sep align, precision=4}]{csv_data_4_tex/mse_r2_table.csv}\caption{Model evaluation results. Full basic-level and frequency model exhibits the best fit to both data sets in terms of variance explained $(r^2)$ and mean squared error (MSE).}
    \label{tab:r2bf}
  \end{table}
\end{center}
...
One can see that middle-of-the-scale control items were sometimes inferred to be closer to high-end of the scale (e.g., the \emph{parrot} is considered relatively light in color relative to other birds) and sometimes closer to the low-end of the scale (e.g., the \emph{laptop} is considered relatively light in weight relative to other electronic devices; Figure~\ref{fig:residuals}, Adjective Endorsement data). These judgments result in different imputed  world knowledge priors (Figure~\ref{fig:worldPriors}) which leads to different comparison class inferences (Figure~\ref{fig:residuals}, Comparison Class Inferences). For example, the \emph{parrot} patterns more like the \emph{seagull} than it does the \emph{crow} in its comparison class inferences: when described as \emph{dark}, it is slightly more likely to be considered \emph{dark for a bird} than when it is described as \emph{light} in color. 
The data set on comparison class inferences, however, exhibits  even more variability than our model, with its particular sources of information, can account for. 
...
The fact that participants naturally produce different responses to the comparison class elicitation task suggests that these inferences can be flexibly adjusted in a spontaneous manner, without an experimenter directly querying about potential alternative comparison classes \cite<cf.>{tessler2017warm}. \ndg{why?}\mht{better?}
