...
Using the case study of gradable adjectives (e.g., \emph{warm}), we extend a Bayesian model of pragmatic inference to reason flexibly about the comparison class and test the qualitative predictions of this model in a large-scale free-production experiment.
We compare several models for inferring the comparison class, finding that a Theory of Mind mechanism is necessary to derive the correct qualitative predictions. 
 We then quantitatively synthesize the model and data using Bayesian data analysis, which further reveals that usage frequency and a preference for basic-level categories are two main factors in comparison class inference.
 \ndg{don't like this sentence:}
...
In the above model, the comparison class variable $c$ takes shape as an another interlocutor's (the speaker's) beliefs \ndg{?}; thus, the inference about the comparison class relies upon a Theory of Mind based reasoning mechanism. 
...
Our primary hypothesis was that usage of a subordinate~vs.~superordinate comparison class would be influenced via an interaction between the general expectations about the category and the polarity of the adjective\ndg{currently this hypothesis isn't totally clear in model section above...?}, which we tested with a maximal Bayesian mixed-effects model.\footnote{
...
\ndg{i feel like one sentence here connecting to the broader question of reference classes would be good... eg maybe something like: people infer the most likely reference class given what their world knowledge tells them about what observations would be remarkable enough for someone to mention? }
