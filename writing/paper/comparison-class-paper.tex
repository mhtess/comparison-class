...
Using the case study of gradable adjectives (e.g., \emph{warm}), we extend a Bayesian model of pragmatic reasoning to reason flexibly about the comparison class and test its qualitative predictions in a large-scale free-production experiment.
We compare models that infer the comparison class with and without a Theory of Mind based reasoning mechanisms, finding that the Theory of Mind mechanism is necessary to derive the correct qualitative predictions. 
...
The problem of determining the comparison class is a special case of the \emph{reference class problem}, originally developed in the domain of probability and statistics when one wants to calculate a probability for a singular future event \cite<i.e., the probability that Joe Biden will be re-elected president in 2024;>{Reichenbach1949, hajek2007reference}.
...
In natural language, the problem of reference classes t (or, comparison classes) is front and center when interpreting \emph{relative statements} (e.g., warm relative to what?).\footnote{
The terms \emph{reference classes} and \emph{comparison classes} seemingly have independent lives in independent literatures, but they are extremely related concepts. Given that our work uses language to gain insight into the reference class problem, we use the terminology \emph{comparison classes} throughout.
}
...
Thus, a literal listener model is insufficient to resolve comparison classes in context and this inference seems to require a Theory of Mind based reasoning mechanism.
