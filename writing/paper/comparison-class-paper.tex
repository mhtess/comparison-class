...
%\DeclareLanguageMapping{american}{american-apa}
  ...
  \title{Warm (for Winter): Inferring comparison classes for gradable adjectives}
...
Comparison classes are a case study in the kinds of representations that sit at the interface of psychological and linguistic theory.
In formal linguistic semantics, the comparison class is a free (underspecified) variable that often gets ``filled in by context'' \cite{bale2008universal, Bale2011, Solt2009}.
Comparison classes need to be inferred because any particular referent of discourse can be conceptualized or categorized in multiple ways, giving rise to multiple possible comparison classes:
...
Psychological and/or communicative (pragmatic) mechanisms presumably are responsible for that filling in process, but how those mechanisms should function to determine a comparison class remains obscure. 
...
On the empirical side, the question of how human listeners decide upon the comparison class has also received little systematic attention.
Empirical work with adults and children has primarily interrogated how judgments and interpretations for relative adjectives like \emph{dark} or \emph{tall} depend upon the statistical details of a pre-determined comparison class  \cite{Barner2008, Qing2014, Schmidt2009, Solt2012}.
Indirect evidence that the comparison class can be flexibly adjusted comes from studies with young children, which find that a directly-modified noun phrase can be used to constrain what objects go into the comparison class: What counts as a \emph{tall pimwit} (a novel category) depends on the distribution of heights of \emph{pimwits} and not the heights of other categories like \emph{daxes} \cite{Barner2008}.
...
A systematic investigation into how human listeners flexibly adjust the comparison class is outstanding. 
...
In this paper, we articulate a general, computational model for inferring comparison classes that integrates psychological, linguistic, and pragmatic representations in a coherent way and makes quantitative predictions about comparison class inferences. 
The model predicts an intuitive interaction between a listener's world knowledge about the referent (e.g., winter days are generally cold; summer days are generally warm) and the valence/polarity of the adjective heard (e.g., \emph{It is warm}~vs.~\emph{It is cold}).
The model also makes fine-grained qualitative predictions that can depend upon two representations posited by the model: probabilistic knowledge about the property in the category (e.g., the distribution over temperature for days in Winter in a particular location) and prior beliefs about the comparison class, a representation that has never been investigated.
We formalize additional predictions about the nature of the comparison class prior and show that it credibly contains elements that encode a basic level bias \cite{Rosch1975} and a smaller effect of frequency.
We test these hypotheses in a large-scale, pre-registered, free-production experiment in which participants are freely generate comparison classes when interpreting adjective phrases in context.
