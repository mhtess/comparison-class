...
ggplot(., aes(x = sub_mu, y = subInterpretation, fill = Adjective))+
  ...
  geom_hline(yintercept = 0.5, lty = 2, color = 'white')+
  xlab("Subordinate Category")+
  ylab("Subordinate comparison class")+
  scale_y_continuous(limits = c(0, 1), breaks = c(0,0.5, 1))+
  theme(#legend.position = "bottom",
        #legend.direction = "horizontal",
        legend.title = element_blank(),
...
make_density_line <- function(mu, sigma){
  bind_rows(lapply(seq(-2, 2, 0.1), function(b){
...
ggplot(., aes(x = s, y = prob, color = utt, alpha = utt))+
...
mp.s2 %>%
  ...
  mutate(sub_mu = factor(sub_mu, 
         ...
         c = factor(c, levels = c("sub", "super"),
                    labels = c("for a basketball player",
                               "for a person"))) %>%
